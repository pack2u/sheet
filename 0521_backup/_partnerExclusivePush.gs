...
/**
 * P열(고유ID) 없는 행에 대해 결정론적 UID를 생성한다.
 * 소스 탭 기준 열: C(2)=일자, D(3)=품목코드, G(6)=수량, H(7)=전화, I(8)=모바일, M(12)=수취인명
 *
 * 동일 행은 항상 동일 UID → 재실행 시 중복 발주 방지
 * 같은 사람이 다른 코드 주문 → 코드가 달라 UID 다름 → 각각 정상 발주
 */
function _pep_deriveDeterministicUid_(row, todayYmd) {
  ...
  // 해시 입력값: 날짜+품목코드+수취인명+전화
  var code      = String(row[_PEP_CODE_COL] || "").replace(/\s/g, "").substring(0, 8) || "X";
  var recipient = String(row[12] || "").replace(/\s/g, "").substring(0, 4) || "U";
  var phone     = String(row[8] || row[7] || "").replace(/[^0-9]/g, "");
  var hashInput = dateStr + code + recipient + phone;
...
function _pep_loadAliasMap_() {
  var result = { byPfxCode: {}, byCode: {}, byPfxCodeMulti: {}, byCodeMulti: {} };
  try {
    ...
    for (var ri = 1; ri < data.length; ri++) {
      ...
      // ★ 세트 지원: 동일 코드 다중 엔트리 수집 (2행 이상이면 세트 구성품으로 인식)
      if (!result.byPfxCodeMulti[pfx + "_" + code]) result.byPfxCodeMulti[pfx + "_" + code] = [];
      result.byPfxCodeMulti[pfx + "_" + code].push(entry);
      if (!result.byCodeMulti[code]) result.byCodeMulti[code] = [];
      result.byCodeMulti[code].push(entry);
...
function partnerPushOrdersToExclusiveForms(silent) {
  ...
  var _tempUidSet_ = _pep_loadTempTabUids_(_tempTab_);
  ...
  // ★ pre-split 세트 감지: 루프 전에 P열 UID별로 서로 다른 품목코드 집계
  // 세트분리 시트가 소스탭에 이미 2행 분리(동일 UID, 다른 코드)해 넣은 경우 감지
  var _preSplitUidCodeMap_ = {}; // uid → { codes: Set, count: number }
  for (var _psi_ = 1; _psi_ < srcAll.length; _psi_++) {
    var _psUid_  = String(srcAll[_psi_][15] || "").trim();
    var _psCode_ = String(srcAll[_psi_][_PEP_CODE_COL] || "").trim();
    if (_psUid_ && _psCode_) {
      if (!_preSplitUidCodeMap_[_psUid_]) {
        _preSplitUidCodeMap_[_psUid_] = { codes: {}, count: 0 };
      }
      if (!_preSplitUidCodeMap_[_psUid_].codes[_psCode_]) {
        _preSplitUidCodeMap_[_psUid_].codes[_psCode_] = true;
        _preSplitUidCodeMap_[_psUid_].count++;
      }
    }
  }
  // count >= 2 이면 해당 UID는 pre-split 세트 → UID|code 복합키 사용
  Logger.log("[PEP] pre-split 세트 UID 감지: " +
    Object.keys(_preSplitUidCodeMap_).filter(function(k){ return _preSplitUidCodeMap_[k].count >= 2; }).length + "건");
  ...
  for (var ri = 1; ri < srcAll.length; ri++) {
    ...
    if (!_rowUid_) {
      // P열 없으면 결정론적 UID 생성 (날짜+코드+수취인+전화끝4)
      // → 같은 행은 항상 같은 UID → 재실행 시 중복 방지
      // → 같은 사람이 다른 코드 주문 시 코드가 달라 각각 정상 발주
      _rowUid_ = _pep_deriveDeterministicUid_(row, today.replace(/-/g, ""));
      row[15] = _rowUid_; // 메모리에만 반영 (소스 시트 P열은 건드리지 않음)
      // → 임시탭 저장 시 P열에 UID가 포함되어 다음 실행 시 중복 방지 작동
      Logger.log("[PEP] R" + (ri + 1) + " 고유ID 없음 → 결정론적 UID 생성: " + _rowUid_);
    }

    // ★ 모든 발주 건 → 임시탭 기록 (협력/비협력 구분 없이, 내부에서 중복 스킵)
    // ★ 성능최적화: 임시탭 배치 버퍼에 적재 (개별 setValues 제거)
    // ★ 세트 지원: 복합키(UID|품목코드) 기준 중복 판별 — 동일 UID라도 품목코드가 다르면 별도 행 허용
    var _tCompositeKey_ = _rowUid_ + "|" + rawCode;
    if (_rowUid_ && _tempTab_ && !_tempUidSet_[_tCompositeKey_]) {
      _tempUidSet_[_tCompositeKey_] = true;
      _tempUidSet_[_rowUid_] = true; // 하위호환: 단순 UID도 등록
      var _tRow_ = [];
      for (var _tci_ = 0; _tci_ < row.length; _tci_++) _tRow_.push(row[_tci_]);
    ...
    // ★ 세트 여부 판별: 두 조건 모두 충족해야 세트로 인식
    //   1) 별칭맵에 동일 코드 2개 이상 엔트리 존재
    //   2) 품목명(rawName) 또는 소스 품목명에 한글 "세트" 포함
    //      ※ 영문 "SET"만 있는 경우는 세트 처리 제외
    var _ak_ = pfx + "_" + rawCode;
    var _setEntries_ = null;
    if (aliasMap.byPfxCodeMulti && aliasMap.byPfxCodeMulti[_ak_] && aliasMap.byPfxCodeMulti[_ak_].length > 1) {
      _setEntries_ = aliasMap.byPfxCodeMulti[_ak_];
    } else if (aliasMap.byCodeMulti && aliasMap.byCodeMulti[rawCode] && aliasMap.byCodeMulti[rawCode].length > 1) {
      _setEntries_ = aliasMap.byCodeMulti[rawCode];
    }
    // 한글 "세트" 포함 여부 — rawName 또는 별칭맵 첫 엔트리의 name 중 하나라도 포함되면 세트
    var _hasKoreanSet_ = false;
    if (_setEntries_ !== null) {
      var _nameToCheck_ = rawName
        || (_setEntries_[0] && _setEntries_[0].name ? _setEntries_[0].name : "")
        || rawCode;
      _hasKoreanSet_ = _nameToCheck_.indexOf("\uc138\ud2b8") !== -1; // "세트"
    }
    var _isSet_ = (_setEntries_ !== null) && _hasKoreanSet_;

    // ★ 전용양식 AX열 기존 UID 직접 비교 — 가장 확실한 중복 방지
    // 우선순위:
    //   1) pre-split 세트 (동일 UID·다른 코드 2행 이상): UID|code 복합키
    //   2) 별칭맵 세트 (_isSet_): _S1 컴포넌트 존재 여부
    //   3) 일반: UID 단일키
    var _isPreSplit_ = !_isSet_ &&
      _preSplitUidCodeMap_[_rowUid_] &&
      _preSplitUidCodeMap_[_rowUid_].count >= 2;
    var _dedupUid_;
    if (_isPreSplit_) {
      _dedupUid_ = _rowUid_ + "|" + rawCode; // pre-split: 복합키
    } else if (_isSet_) {
      _dedupUid_ = _rowUid_ + "_S1";          // 별칭맵 세트: 기존 로직 유지
    } else {
      _dedupUid_ = _rowUid_;                   // 일반
    }
    if (
      _dedupUid_ &&
      cache[pfx].existingUids &&
      (cache[pfx].existingUids[_dedupUid_] ||
       // 하위호환: pre-split 복합키가 없더라도 단순 UID가 등록돼 있으면 스킵
       (_isPreSplit_ && cache[pfx].existingUids[_rowUid_]))
    ) {
      skipUid++;
      continue; // 이미 발주된 행 → 전용양식 스킵
    }

    // 별칭 조회 (품목코드·품목명 변환) — 세트일 때는 첫 번째 구성품 기준
    var vendorSku = "",
      vendorName = "";
    try {
      var ak = _ak_;
      var ae = _isSet_ ? _setEntries_[0] : (aliasMap.byPfxCode[ak] || aliasMap.byCode[rawCode]);
      if (ae) {
        vendorSku = ae.sku || "";
        vendorName = ae.name || "";
      }
    ...
    // ★ AX열(index 49) — 고유ID 기입 (기존 헤더 배열 수정 없이 안전한 열 사용)
    // P열(row[15]) = 사방넷주문번호 = 고유ID
    // 우선순위:
    //   pre-split 세트: UID|code 복합키  (2행 모두 AX에 고유값 보장)
    //   별칭맵 세트:    UID_S1           (기존 로직 유지)
    //   일반:           UID
    var _pepUid_ = _isPreSplit_
      ? (_rowUid_ + "|" + rawCode)
      : (_isSet_ ? (_rowUid_ + "_S1") : _rowUid_);
    if (_pepUid_) {
    ...
    try {
      ...
      // ★ 성능최적화: 배치 버퍼에 적재 (개별 setValues 제거)
      cache[pfx].pendingRows.push({ outRow: outRow, dmCols: dmCols });
      cache[pfx].nextRow = nextRow + 1; // 다음 행 위치 갱신

      pushed++;
      // ★ 발주 성공 → existingUids에 추가 (동일 실행 내 중복 방지)
      // dedup 키와 동일한 규칙으로 등록 (_dedupUid_ 재사용)
      if (_dedupUid_ && cache[pfx].existingUids) {
        cache[pfx].existingUids[_dedupUid_] = true;
      }

      // ★ 세트 나머지 구성품 Push (index 1, 2 …)
      if (_isSet_) {
        for (var _si_ = 1; _si_ < _setEntries_.length; _si_++) {
          var _setUid_ = _rowUid_ + "_S" + (_si_ + 1);
          if (cache[pfx].existingUids[_setUid_]) continue; // 이미 Push됨

          var _setAe_ = _setEntries_[_si_];
          var _setOutRow_ = outRow.slice(); // 기존 outRow(배송정보 포함) 복사

          // 구성품별 업체코드/업체품목명 덮어쓰기
          if (_setAe_) {
            if (effSkuCol != null) {
              _setOutRow_[effSkuCol] = directMap.vendorSkuCol != null
                ? (_setAe_.sku || rawCode)
                : (_setAe_.sku || _setOutRow_[effSkuCol]);
            }
            if (effNameCol != null) {
              _setOutRow_[effNameCol] = directMap.vendorNameCol != null
                ? (_setAe_.name || String(row[_PEP_ITEM_COL] || "").trim() || rawCode)
                : (_setAe_.name || _setOutRow_[effNameCol]);
            }
          }

          // 순번(seqCol) 재할당
          if (directMap.seqCol != null && cache[pfx].nextSeq != null) {
            _setOutRow_[directMap.seqCol] = cache[pfx].nextSeq++;
          }

          // AX열 UID 덮어쓰기
          while (_setOutRow_.length <= 49) _setOutRow_.push("");
          _setOutRow_[49] = _setUid_;

          // HR 단가 재계산 (구성품별 단가 상이 가능)
          if (pfx === "HR" && _setAe_) {
            try {
              var _sPriceVat_ = _setAe_.priceVat > 0 ? _setAe_.priceVat
                : (_setAe_.price > 0 ? _setAe_.price + (_setAe_.vat > 0 ? _setAe_.vat : Math.round(_setAe_.price * 0.1)) : 0);
              if (_sPriceVat_ > 0) {
                var _sPriceEx_ = Math.round(_sPriceVat_ / 1.1);
                var _sVatUnit_ = _sPriceVat_ - _sPriceEx_;
                var _sQty_ = parseFloat(_setOutRow_[24]) || 0;
                _setOutRow_[25] = _sPriceEx_;
                _setOutRow_[26] = _sQty_ * _sPriceVat_;
                _setOutRow_[28] = _sQty_ * _sPriceEx_;
                _setOutRow_[29] = _sQty_ * _sVatUnit_;
              }
            } catch (eSetHR) {}
          }

          cache[pfx].pendingRows.push({ outRow: _setOutRow_, dmCols: dmCols });
          cache[pfx].nextRow++;
          pushed++;
          cache[pfx].existingUids[_setUid_] = true;
          Logger.log("[PEP] 세트 구성품 Push: UID=" + _setUid_
            + " sku=" + (_setAe_ ? _setAe_.sku : "")
            + " name=" + (_setAe_ ? _setAe_.name : ""));
        }
      }
    } catch (eW) {
      errorLogs.push(
  ...
  if (_tempPendingRows_.length > 0 && _tempTab_) {
    try {
      ...
      _tempTab_.getRange(tStartRow, 1, _tempPendingRows_.length, tMaxCols).setValues(_tempPendingRows_);
  ...
  var allDmPfx =
    typeof _PEP_VENDOR_DIRECT_MAP_ !== "undefined"
      ...
      : [];
  for (var pi = 0; pi < allDmPfx.length; pi++) {
    var pp = allDmPfx[pi];
    var mapped = prefixToFile[pp];
    pfxMapLines.push(
      "  " + pp + ": " + (mapped ? mapped.name : "❌ 파일 없음"),
    );
  }
...
function _pep_ensureNonPartnerTempTab_(ss) {
  ...
  if (!tab) {
    ...
    var a1 = String(tab.getRange("A1").getValue() || "").trim();
    if (a1 !== "상태") {
      tab
        .getRange(1, 1, 1, _PEP_NON_PARTNER_TEMP_HEADERS_.length)
        .setValues([_PEP_NON_PARTNER_TEMP_HEADERS_]);
...
// 임시탭 P열(15)+D열(3) 복합키 세트 로드 (중복 방지용)
// ★ 세트 지원: 동일 UID라도 품목코드가 다르면 별도 행으로 허용 (복합키 = UID|code)
function _pep_loadTempTabUids_(tab) {
  var uidSet = {};
  if (!tab || tab.getLastRow() < 2) return uidSet;
  var lastCol = Math.max(tab.getLastColumn(), 16);
  var vals = tab.getRange(2, 1, tab.getLastRow() - 1, lastCol).getValues();
  for (var i = 0; i < vals.length; i++) {
    var uid  = String(vals[i][15] || "").trim(); // P열(15): 사방넷주문번호 = 고유ID
    var code = String(vals[i][3]  || "").trim(); // D열(3): 품목코드
    if (uid) {
      var compositeKey = uid + "|" + code;
      uidSet[compositeKey] = true;
      uidSet[uid] = true; // 하위호환: 단순 UID 조회도 유지
    }
  }
  return uidSet;
}

// 임시탭에 행 추가 (UID|품목코드 복합키 기준 중복 스킵)
// ★ 세트 지원: 동일 UID + 다른 품목코드 조합은 별도 행으로 허용
// ★ 소스 행 그대로 복사 + W(22)=업체prefix + X(23)=송장번호(빈칸)
function _pep_appendToNonPartnerTempTab_(row, pfx, tab, uidSet, nowStr) {
  var uid  = String(row[15] || "").trim(); // P열(15): 사방넷주문번호 = 고유ID
  var code = String(row[3]  || "").trim(); // D열(3): 품목코드
  if (!uid || !tab) return;
  var compositeKey = uid + "|" + code;
  if (uidSet[compositeKey]) return; // 이미 있으면 스킵
  uidSet[compositeKey] = true;
  uidSet[uid] = true; // 하위호환
  var srcRow = [];
  for (var ci = 0; ci < row.length; ci++) srcRow.push(row[ci]);
  var newRow = srcRow.concat([pfx, ""]); // 소스행 + 업체prefix(W) + 송장번호빈칸(X)
...
function _pep_initVendorCache_(pfx, fileInfo, directMap) {
  ...
  try {
    ...
    // ★ 전용양식 AX열(50번째, index 49) 기존 고유ID 사전 로드 — 중복 발주 방지
    // ★ tab.getLastRow() 직접 사용 — _pep_findActualLastRow_(B~H 의존)보다 정확
    //   B~H가 비어도 AX에만 값이 있는 전날 건을 누락 없이 로드
    var existingUids = {};
    try {
      var axLr = tab.getLastRow(); // 실제 최말단 행 (AX 포함 전체)
      if (axLr >= 2) {
        var axVals = tab.getRange(2, 50, axLr - 1, 1).getValues(); // AX열(50)
        for (var ai = 0; ai < axVals.length; ai++) {
          var axUid = String(axVals[ai][0] || "").trim();
          if (axUid) {
            existingUids[axUid] = true;
            // 하위호환: pre-split 복합키(UID|code)에서 base UID도 등록
            var _pipeIdx_ = axUid.indexOf("|");
            if (_pipeIdx_ > 0) {
              existingUids[axUid.substring(0, _pipeIdx_)] = true;
            }
          }
        }
        Logger.log(
          "[PEP] " + pfx +
          " 전용양식 AX열 기존UID=" + Object.keys(existingUids).length + "건 로드 (tab.getLastRow=" + axLr + ")",
        );
      }
    } catch (eAx) {
      Logger.log("[PEP] " + pfx + " AX열 로드 실패: " + eAx.message);
    }
    return {
      ss: ss,
      tab: tab,
      ...
      batchColor: null,
      existingUids: existingUids, // ★ AX열 기존 UID Set
