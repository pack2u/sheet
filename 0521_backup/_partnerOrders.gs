...
function _po_applyHubDesign(hubTab) {
  try {
    var hRange = hubTab.getRange("A2:O5000");
    var rules = [];
    rules.push(
...
function partnerCollectOrders(opt_noWriteBack) {
  ...
  if (!hubWasEmpty) {
    ...
    for (var ei = 0; ei < hubAllData.length; ei++) {
      ...
      if (eName && eCd) existingKeySet[eName + "_" + eShrt + "_" + eCd] = true;
    }
    ...
    try {
        ...
        if (vs) {
          ...
          if (vs.getLastRow() >= 3) {
            ...
            for (var p = 0; p < pData.length; p++) {
              var pCode = String(pData[p][2] || "").replace(/\s/g, "");
      ...
      for (var ti = 0; ti < allTabs.length; ti++) {
        ...
        for (var r = 1; r < data.length; r++) {
          var code =
            cMap.code !== -1 ? String(data[r][cMap.code] || "").trim() : "";
          var itemName =
          ...
          var isDup =
            existingIds[uid] ||
            (!hubWasEmpty && recipient && code && existingKeySet[dupKey]);
          if (isDup) {
            skipped++;
            continue;
          }
          existingIds[uid] = true;
          if (recipient && code) existingKeySet[dupKey] = true;

          // 주문일자 정규화
          var orderDate = cMap.date !== -1 ? data[r][cMap.date] : "";
          var dateStr = "";
 ...
 * 매칭 우선순위: 고유ID → 이름+끝4 → 이름+앞7 (마스킹 대응)
 */
function partnerFetchInvoices() {
  ...
  // ── ★ 합배송 전용 시트의 이름+전화 키를 별도 Set으로 수집 (합배송 판정 기준) ──
  ...
  try {
    ...
    if (_csTab && _csTab.getLastRow() > 1) {
      var _csLc = _csTab.getLastColumn();
      var _csData = _csTab
    ...
    for (var pfi = 0; pfi < pFiles.length; pfi++) {
      try {
        ...
        for (var pti = 0; pti < ptabs.length; pti++) {
          ...
          if (ptName.indexOf("전용양식") === -1) continue;
          var ptab = ptabs[pti];
  ...
  for (var r = 0; r < hubData.length; r++) {
    ...
    if (invoiceMap[hubUid]) {
      ...
      if (uidCandidates.length > 0) {
        var needSlots0 = _pt_getRequiredParcelSlots(hubData[r]);
  ...
  for (var r = 0; r < hubData.length; r++) {
    ...
    if (existingInv) {
      continue; // alreadyHas는 위에서 이미 카운트됨
    }
    if (isTerminalOrderStatus_(String(hubData[r][14] || ""))) continue;
    var st = String(hubData[r][14] || "").trim();
    if (st === "발송완료") continue;

    // ★ 고유ID가 있는 행은 고유ID 매칭을 먼저 시도했으나 실패한 경우,
    //    이름+전화 기반 매칭도 허용 (로젠 원본에는 고유ID가 없으므로 필요)
    //    단, 합배송 등 오매칭 방지를 위해 그룹핑 1차 엄격 매칭으로 제어
    var hubUidForGroup = String(hubData[r][2] || "").trim();
    // (고유ID 있어도 이름+전화 매칭 가능 — 고유ID는 1차 패스에서 이미 시도됨)
  ...
  for (var ug = 0; ug < unmatchedGroups.length; ug++) {
    ...
    for (var gx2 = 0; gx2 < uRows.length; gx2++) {
      var ri2 = uRows[gx2];
      var nm2 = String(hubData[ri2][7] || "").trim();
      var ph2 = String(hubData[ri2][8] || "").replace(/[^0-9]/g, "");
      var nmN2 = nm2.replace(/[^가-힣ㄱ-ㅎㅏ-ㅣa-zA-Z0-9]/g, "");
      ...
      // 이름 단독 (원본)
      if (mergedMatched2.length === 0 && nm2.length >= 2) {
        var nrKey2 = "NR_" + nm2;
        if (invoiceMap[nrKey2])
          mergedMatched2 = mergedMatched2.concat(invoiceMap[nrKey2]);
      }
      // 이름 단독 (정규화)
      if (mergedMatched2.length === 0 && nmN2.length >= 2) {
        var nnKey2 = "N_" + nmN2;
        if (invoiceMap[nnKey2])
          mergedMatched2 = mergedMatched2.concat(invoiceMap[nnKey2]);
      }
    }
  ...
  if (hubChanged) {
    hubTab.getRange(2, 1, hubData.length, _PO_HUB_HEADERS.length).setValues(hubData);
    SpreadsheetApp.flush();
  }
  ...
  try {
    // 매칭 성공한 건의 복합키(코드+이름+전화)→송장번호 맵 구축
    var hubInvoiceByKey = {};
    for (var ui2 = 0; ui2 < writeUpdates.length; ui2++) {
      ...
      var hCode2 = String(hubData[hubRow2][4] || "").trim(); // E열: 이카운트코드
      var hName2 = String(hubData[hubRow2][7] || "").trim(); // H열: 수취인
      var hPhone2 = String(hubData[hubRow2][8] || "").replace(/[^0-9]/g, ""); // I열: 전화번호
      var hUid2 = String(hubData[hubRow2][2] || "").trim(); // C열: 고유ID
  ...
  // ★ 임시탭 고유ID(사방넷주문번호) 기반 비협력업체 건 K열에 송장번호 기록
  try {
    _po_checkNonPartnerTempTabMatches_(invoiceMap, scannedLogs);
  } catch (eNp) {
      ...
      for (var ti = 0; ti < allTabs.length; ti++) {
        ...
        for (var r = 1; r < data.length; r++) {
          ...
          if (p.memoOnly) {
            ...
            if (!invSame && invCol !== -1) {
              data[r][invCol] = p.invoice;
              tabChanged = true;
            }
            if (!stSame && cMap.status !== -1) {
              data[r][cMap.status] = "발송완료";
              tabChanged = true;
            }
            // 허브 적요 → 발주탭 적요 배포
            if (p.hubMemo && cMap.note !== -1) {
        ...
        // ★ 성능최적화: 변경 있으면 data 배열 전체를 한 번에 기록
        if (tabChanged) {
          tab.getRange(1, 1, data.length, lc).setValues(data);
          SpreadsheetApp.flush();
        }
      }
    } catch (e) {
...
function partnerApplyVoidedInvoices(silent) {
  ...
  for (var r = 0; r < hubData.length; r++) {
    ...
    // ★ 성능최적화: hubData 배열에 직접 반영 (개별 setValue 제거)
    hubData[r][13] = newInv; // 송장번호 갱신 (빈 문자열 가능)

    // 송장번호가 완전히 비었으면 상태를 "접수완료"로 복원
    if (!newInv) {
 ...
 */
// ★ 비협력업체 임시탭(대리발송_임시기록)
//   A열(고유ID = 사방넷주문번호) 기반으로 invoiceMap에서 송장 찾아 W열(22)에 기록
function _po_checkNonPartnerTempTabMatches_(invoiceMap, scannedLogs) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var tempTab = ss.getSheetByName("대리발송_임시기록");
  if (!tempTab || tempTab.getLastRow() < 2) {
    scannedLogs.push("[비협력임시탭] 데이터 없음 스킵");
    return;
  }
  var tempLr = tempTab.getLastRow();
  var tempLc = Math.max(tempTab.getLastColumn(), 23); // W열(23번째)까지 읽기
  var tempData = tempTab.getRange(2, 1, tempLr - 1, tempLc).getValues();
  var totalNp = 0, matchedNp = 0, noMatchNp = 0;
  var updates = [];
  for (var ti = 0; ti < tempData.length; ti++) {
    var tUid = String(tempData[ti][15] || "").trim(); // P(15): 사방넷주문번호=고유ID
    if (!tUid) continue;
    totalNp++;
    var existInv = String(tempData[ti][23] || "").trim(); // X(23): 송장번호
    if (existInv) { matchedNp++; continue; }

    // invoiceMap에서 사방넷주문번호 키로 직접 조회
    var found = invoiceMap[tUid];
    if (found && found.length > 0) {
      var bestInv = "";
      for (var fi = 0; fi < found.length; fi++) {
        var candidate = String(found[fi].invRaw || "").trim();
        if (candidate) { bestInv = candidate; break; }
      }
      if (bestInv) {
        updates.push({ row: ti + 2, inv: bestInv });
        matchedNp++;
      } else { noMatchNp++; }
    } else { noMatchNp++; }
  }
  // W열(23번째 열)에 송장번호 일괄 기록
  for (var ui = 0; ui < updates.length; ui++) {
    tempTab.getRange(updates[ui].row, 24).setValue(updates[ui].inv); // X열(24번째)
  }
  if (updates.length > 0) SpreadsheetApp.flush();
  scannedLogs.push("[비협력 임시탭] 전체: " + totalNp + "건 / 송장기록: " + matchedNp + "건 / 미매칭: " + noMatchNp + "건");
}
...
function _po_pushTempTabMatchedToNonPartnerSheet_(scannedLogs) {
  ...
  for (var ti = 0; ti < tempData.length; ti++) {
    ...
    outRow[_UM_COL_INAME]     = itemName;  // W(22): 물품명
    ...
    try {
      ...
      for (var ri = 1; ri < ptData.length; ri++) {
        ...
        outRow[_UM_COL_INAME] =
          itemIdx >= 0 ? String(row[itemIdx] || "").trim() : ""; // W: 물품명
        outRow[_UM_COL_MSG] =
...
function _po_onEditVoidInvoiceAutoFill_(e) {
  ...
  if (sheet.getName() !== _PO_VOID_TAB_NAME) return;
