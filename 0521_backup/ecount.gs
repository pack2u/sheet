...
/**
 * 상품정보 탭의 시트/범위 보호 목록을 보여 준다.
 * 동기화(moveToEcount)는 실행 중인 계정 권한으로 A1·Z3·6행 이하 데이터열 등을 쓰므로,
 * '편집 제한' 규칙에 그 계정이 빠져 있으면 타인/본인 모두 동일하게 실패한다.
 */
function diagnoseProductInfoSheetProtections() {
  var ui = SpreadsheetApp.getUi();
  var ss = SpreadsheetApp.getActiveSpreadsheet() || getSafeActiveSS();
  var sh = ss.getSheetByName("상품정보");
  if (!sh) {
    ui.alert("'상품정보' 탭이 없습니다.");
    return;
  }

  function editorsLine_(p) {
    try {
      if (typeof p.isWarningOnly === "function" && p.isWarningOnly()) {
        return "유형: 편집 시 경고만";
      }
    } catch (w) {}
    try {
      ...
      if (!eds || eds.length === 0) {
        return "편집 허용 목록: (비어 있음 — 도메인 전체 또는 소유자만 등 정책일 수 있음)";
      }
      var parts = [];
      var cap = Math.min(eds.length, 12);
      for (var i = 0; i < cap; i++) {
        try {
          parts.push(eds[i].getEmail());
        } catch (e2) {
          parts.push("(이메일 조회 불가)");
        }
      }
      if (eds.length > cap) parts.push("… 외 " + (eds.length - cap) + "명");
      return "편집 허용: " + parts.join(", ");
    } catch (e) {
      return "편집자 목록 조회 실패";
    }
  }
  ...
  var eff = "(확인 불가)";
  try {
    eff = Session.getEffectiveUser().getEmail() || "(비공개)";
  } catch (e0) {}

  var lines = [];
  lines.push("※ 스크립트는 항상 '아래 실행 계정'의 권한으로 시트에 씁니다.");
  lines.push("※ 파일 소유자라도, 보호 규칙에서 제외되면 같은 오류가 납니다.");
  lines.push("");
  lines.push("실행 계정(EffectiveUser): " + eff);
  lines.push("");

  var sheetPs = sh.getProtections(SpreadsheetApp.ProtectionType.SHEET);
  var rangePs = sh.getProtections(SpreadsheetApp.ProtectionType.RANGE);

  lines.push("[시트 전체 보호] " + sheetPs.length + "건");
  for (var si = 0; si < sheetPs.length; si++) {
    var sp = sheetPs[si];
    var desc = "";
    try {
      desc = sp.getDescription() || "";
    } catch (e1) {}
    lines.push("  • " + (desc || "(설명 없음)") + " | " + editorsLine_(sp));
  }

  lines.push("");
  lines.push("[범위 보호] " + rangePs.length + "건");
  var maxRanges = 35;
  for (var ri = 0; ri < rangePs.length; ri++) {
    if (ri >= maxRanges) {
      lines.push("  … 이하 생략(" + (rangePs.length - maxRanges) + "건). 시트에서 데이터 > 보호된 시트 및 범위 로 확인.");
      break;
    }
    var rp = rangePs[ri];
    var a1 = "";
    try {
      a1 = rp.getRange().getA1Notation();
    } catch (e3) {
      a1 = "?";
    }
    var rdesc = "";
    try {
      rdesc = rp.getDescription() || "";
    } catch (e4) {}
    lines.push("  • " + a1 + " | " + (rdesc || "(설명 없음)") + " | " + editorsLine_(rp));
  }

  lines.push("");
  lines.push("동기화 4단계(moveToEcount)가 쓰는 위치 예: A1, Z3, 6행~·매핑열(A,B,G,…) 등.");
  lines.push("해당 셀이 위 '편집 제한'에 걸리면 ⚙️에서 이 점검으로 원인 확인.");

  var text = lines.join("\n");
  if (text.length > 4500) text = text.slice(0, 4400) + "\n…(메시지 길이 제한으로 잘림)";
  ui.alert("상품정보 · 보호 범위 점검", text, ui.ButtonSet.OK);
}
...
function getOrCreatePermissionAuditSheet_() {
  var ss = SpreadsheetApp.getActiveSpreadsheet() || getSafeActiveSS();
  var name = "권한진단로그";
  ...
  if (sh.getLastRow() < 1) {
    ...
    var curr = sh.getRange(1, 1, 1, headers.length).getValues()[0];
    var mismatch = false;
    for (var i = 0; i < headers.length; i++) {
      if (String(curr[i] || "") !== headers[i]) {
        mismatch = true;
        break;
      }
    }
  ...
  sh.getRange(1, 1, 1, headers.length).setBackground("#1f4e78").setFontColor("white").setFontWeight("bold");
  sh.setFrozenRows(1);
  return sh;
}

function getBasicExecutionIdentity_() {
...
function showFirstRunPermissionGuide() {
  var ui = SpreadsheetApp.getUi();
  var lines = [];
  lines.push("처음 실행 권한 안내");
  lines.push("");
  lines.push("이 문서는 메뉴 기능 실행 시 Google 권한 승인이 필요할 수 있습니다.");
  lines.push("- 계정별 최초 1회 승인 후에는 동일 기능에서 반복 팝업이 줄어듭니다.");
  lines.push("- 팝업에서 [확인] → 계정 선택 → [허용] 순서로 진행하세요.");
  lines.push("");
  lines.push("팝업이 계속 뜨는 경우");
  lines.push("1) 다른 구글 계정으로 열려 있음");
  lines.push("2) 해당 계정이 편집 권한이 아님");
  lines.push("3) 브라우저에 오래 열린 탭(구버전 세션) 사용 중");
  lines.push("");
  lines.push("권장");
  lines.push("- 계정 확인 후 탭 새로고침(F5) 뒤 재실행");
  lines.push("- 필요 시 ⚙️ 운영/진단 > 권한 일괄 점검 실행");
  ui.alert(lines.join("\n"));
}

function runUserPermissionSelfCheck() {
  // 기본값은 빠른 진단(Drive 미조회)으로 동작시켜 불필요한 추가 권한 요청을 줄인다.
  var p = getBasicExecutionIdentity_();
  var loginProbe = probeEcountLoginPermission_();

  var templateOk = null;
  var folderOk = null;
  var templateMsg = "SKIP(빠른진단)";
  var folderMsg = "SKIP(빠른진단)";

  var isAdmin = false;
  try {
    if (typeof isAdminUser_ === "function") isAdmin = isAdminUser_();
  } catch (eAdmin) {}

  var summaryParts = [];
  if (!isAdmin) summaryParts.push("관리자아님");
  if (!loginProbe.ok) summaryParts.push("이카운트로그인실패");
  summaryParts.push("Drive상세검사생략");
  var summary = summaryParts.length ? summaryParts.join(", ") : "정상";

  try {
    var sh = getOrCreatePermissionAuditSheet_();
    ...
    var row = [
      ...
      p.activeUser || "",
      p.owner || "",
      p.isOwner === null ? "-" : (p.isOwner ? "Y" : "N"),
      isAdmin ? "Y" : "N",
      loginProbe.ok ? "OK" : "FAIL: " + loginProbe.message,
      templateOk === null ? templateMsg : (templateOk ? "OK" : "FAIL: " + templateMsg),
      folderOk === null ? folderMsg : (folderOk ? "OK" : "FAIL: " + folderMsg),
      summary,
    ];
  ...
  var lines = [];
  lines.push("권한 자가진단 결과");
  lines.push("- 실행계정: " + p.effectiveUser);
  lines.push("- 시트소유자 동일: 빠른 진단에서는 미검사");
  lines.push("- 관리자 등록: " + (isAdmin ? "YES" : "NO"));
  lines.push("- 이카운트 로그인: " + (loginProbe.ok ? "OK" : "FAIL"));
  lines.push("- 템플릿 접근: 빠른 진단에서는 미검사");
  lines.push("- 배포 폴더 접근: 빠른 진단에서는 미검사");
  lines.push("- 요약: " + summary);
  lines.push("");
  lines.push("※ Drive 상세 점검은 관리자용 고급 점검 메뉴에서만 실행하세요.");
  lines.push("");
  lines.push("※ 결과가 '권한진단로그' 시트에 저장되었습니다.");
...
function runPostProcessErrorCheck() {
  var ss = SpreadsheetApp.getActiveSpreadsheet() || getSafeActiveSS();
  var ui = SpreadsheetApp.getUi();
  var lines = [];
  var issues = [];

  function sheetStatus_(name) {
    var sh = ss.getSheetByName(name);
    if (!sh) return { ok: false, msg: "시트 없음" };
    if (sh.getLastRow() < 2) return { ok: true, msg: "데이터 없음(정상)" };
    return { ok: true, msg: "존재" };
  }

  function countRecentFailures_(sheetName, sampleLimit) {
    var sh = ss.getSheetByName(sheetName);
    if (!sh || sh.getLastRow() < 2) return { count: 0, samples: [] };
    var lastRow = sh.getLastRow();
    var width = sh.getLastColumn();
    var readRows = Math.min(Math.max(sampleLimit || 200, 50), lastRow - 1);
    var start = lastRow - readRows + 1;
    var vals = sh.getRange(start, 1, readRows, width).getDisplayValues();
    var c = 0;
    var s = [];
    var re = /(FAIL|실패|ERROR|예외|RUNTIME|OPEN_FAIL|PARTIAL)/i;
    for (var i = 0; i < vals.length; i++) {
      var rowText = vals[i].join(" | ");
      if (re.test(rowText)) {
        c++;
        if (s.length < 5) s.push("- [" + sheetName + "] " + rowText.substring(0, 160));
      }
    }
    return { count: c, samples: s };
  }

  function checkArchiveTabHeaders_() {
    var tabs = ss.getSheets();
    var missing = [];
    for (var i = 0; i < tabs.length; i++) {
      var n = tabs[i].getName();
      if (!/^\(\d{4}년 \d{1,2}월\) 발주 마감$/.test(String(n))) continue;
      var lc = tabs[i].getLastColumn();
      if (lc < 1) continue;
      var h = tabs[i].getRange(4, 1, 1, lc).getDisplayValues()[0];
      var names = h.map(function(x){ return String(x || "").trim(); });
      if (names.indexOf("취소") === -1 || names.indexOf("반품") === -1) {
        missing.push(n);
      }
    }
    return missing;
  ...
  lines.push("처리 후 오류 점검");
  lines.push("");

  var s1 = sheetStatus_("업데이트실행로그");
  var s2 = sheetStatus_("자동화실행로그");
  var s3 = sheetStatus_("이카운트-판매업로드오류");
  var s4 = sheetStatus_("권한진단로그");
  lines.push("1) 필수 로그 시트");
  lines.push("- 업데이트실행로그: " + s1.msg);
  lines.push("- 자동화실행로그: " + s2.msg);
  lines.push("- 이카운트-판매업로드오류: " + s3.msg);
  lines.push("- 권한진단로그: " + s4.msg);
  lines.push("");

  lines.push("2) 최근 실패/오류 스캔 (최근 200행)");
  var c1 = countRecentFailures_("업데이트실행로그", 200);
  var c2 = countRecentFailures_("자동화실행로그", 200);
  var c3 = countRecentFailures_("이카운트-판매업로드오류", 200);
  lines.push("- 업데이트실행로그 FAIL 징후: " + c1.count + "건");
  lines.push("- 자동화실행로그 FAIL 징후: " + c2.count + "건");
  lines.push("- 판매업로드오류 FAIL 징후: " + c3.count + "건");
  lines.push("");

  if (c1.count > 0) issues = issues.concat(c1.samples);
  if (c2.count > 0) issues = issues.concat(c2.samples);
  if (c3.count > 0) issues = issues.concat(c3.samples);

  var missingArchiveHeaders = checkArchiveTabHeaders_();
  lines.push("3) 월마감 탭 구조 점검");
  lines.push("- 취소/반품 헤더 누락 탭: " + missingArchiveHeaders.length + "개");
  if (missingArchiveHeaders.length > 0) {
    lines.push("  · " + missingArchiveHeaders.slice(0, 5).join(", "));
  }
  lines.push("");

  if (missingArchiveHeaders.length > 0) {
    issues.push("- 월마감 헤더 누락 탭 " + missingArchiveHeaders.length + "개 (메뉴: 월별 마감 탭 레이아웃 보정 실행 권장)");
  }

  lines.push("4) 종합");
  if (issues.length === 0) {
    lines.push("✅ 즉시 확인된 오류 징후 없음");
  } else {
    lines.push("⚠ 확인 필요 이슈: " + issues.length + "건");
    for (var k = 0; k < Math.min(issues.length, 8); k++) lines.push(issues[k]);
  }
  lines.push("");
  lines.push("※ 상세 로그는 각 로그 시트에서 확인하세요.");
...
// ═══════════════════════════════════════════
//  Admin 래퍼 (메뉴에 등록만 되고 함수 없어서 오류 발생하던 항목)
// ═══════════════════════════════════════════
