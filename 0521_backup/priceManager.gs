...
function updateAllVendorSheets(opts) {
  ...
  try {
    ...
    if (hasMore) {
      ...
      props.deleteProperty(VENDOR_UPDATE_CURSOR_KEY);
    }

    if (processedThisRun > 0) {
      var elapsed = new Date().getTime() - runStartMs;
      var perFile = elapsed / processedThisRun;
      var prev = parseFloat(props.getProperty(VENDOR_UPDATE_AVG_MS_KEY) || "0");
      var ema = prev > 0 ? prev * 0.7 + perFile * 0.3 : perFile;
      props.setProperty(VENDOR_UPDATE_AVG_MS_KEY, String(Math.round(ema)));
    }

    var endMsg =
      "✅ 총 " +
      updatedCount +
      "개의 배포 시트가 성공적으로 업데이트되었습니다.\n" +
      "- 대상 탐색: " +
      targetCount +
      "개\n" +
      "- 일반: " +
