 ...
 * 중요도 — 순서가 곧 정렬 우선순위.
 *
 * ★ 2026-09-04: 맨 앞에 「공지」를 넣었다.
 *   "금일 판매불가 상품"처럼 모두가 먼저 봐야 하는 글을 위한 칸이다.
 *   정렬이 이 배열 순서를 그대로 쓰므로 공지는 자동으로 맨 위에 선다.
 *   긴급과 다른 점: 긴급은 "이 건을 처리해 달라"고, 공지는 "오늘 이걸 알고
 *   일해 달라"고 말한다. 그래서 완료로 닫는 대신 유효기간이 지나면 내린다.
 */
var _CS_HB_LEVELS_ = ["공지", "긴급", "주의", "일반"];
