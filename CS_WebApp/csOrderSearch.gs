...
function updateReturnLedgerStatus(payload) {
  ...
  try {
    ...
    /*  ★ 송장이 생겼으면 단계도 같이 간다 ★  (2026-09-20)
        ...
        ★ retInvSaved 가 아니라 retInvIn 으로 본다 ★  (2026-09-22)
        retInvSaved 는 «칸의 값이 바뀌었을 때»만 찬다. 그래서 송장이 이미
        적혀 있는 줄(롯데 회수 접수가 먼저 적어 둔 줄, 또는 같은 번호를
        다시 저장한 줄)은 「접수」에 그대로 머물렀다.
        여기서 보려는 것은 «값이 바뀌었나»가 아니라 «회수 송장이 있나»다.

        상태만 바꾸러 온 부름(retInvIn 이 비어 있다)은 건드리지 않는다 —
        사람이 일부러 「접수」로 되돌리는 것을 뒤엎으면 안 된다.  */
    /*  ★ 끝난 건은 «뒤로» 끌지 않는다 ★  (2026-09-22)
        _cs_returnStage_ 는 「이카운트OK」를 0(앞 단계)으로 돌려준다 — 그 함수는
        «완료/환불»이라는 낱말로만 끝을 가리기 때문이다. 끝 판정의 주인은
        _cs_isReturnDoneMark_ 이므로 여기서도 그것에 물어본다.
        안 물어보면 이미 끝낸 건에 송장을 적는 순간 「반품송장」으로 되돌아간다. */
    if (retInvIn && _cs_isBeforePickup_(status) && !_cs_isReturnDoneMark_(status)) {
