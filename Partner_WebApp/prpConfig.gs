...
/* ★ 2026-09-11: 5분 → 90초 ★
     > "서로 같이 반응도 안되"
   CS 가 상태를 바꿔도 업체 화면은 최대 5분 옛 값이었다. 스크립트 캐시는
   프로젝트마다 따로라 CS 쪽에서 이 캐시를 비울 방법이 없다 —
   기다리는 시간을 줄이는 것이 유일한 손잡이다.
   대장 읽기가 무겁지 않고 업체 수도 적어 90초는 감당된다.
   더 급하면 화면의 「새로 불러오기」가 캐시를 건너뛴다. */
var PRP_CACHE_TTL = 90;
...
/**
 * ★ 처음 열었을 때 골라져 있는 택배사 ★  (2026-09-18)
 *
 *   > "업체 반품접수 새로 만들기에서 수거택배사를 기본 로젠택배로 해줘"
 *
 * 2026-09-11 에 롯데 → 로젠으로 바꿨다. 목록 첫째가 그냥 골라지던 탓에
 * 업체가 손대지 않으면 CJ대한통운으로 접수됐다 — 우리가 안 쓰는 곳이다.
 *
 * ★ 목록 차례를 바꾸지 않는다 ★ 차례는 업체가 찾기 좋은 순서고,
 *   기본값은 우리 사정이다. 둘을 한 값으로 묶으면 다음에 또 엉킨다.
 * 목록에 없는 값을 적으면 화면이 못 고르니, 반드시 위 목록 안의 것으로.
 */
var PRP_PICKUP_DEFAULT = "로젠택배";
