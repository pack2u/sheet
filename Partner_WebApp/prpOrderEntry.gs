...
function prpSubmitOrders(sid, payload) {
  ...
  for (var i = 0; i < rowsIn.length; i++) {
    ...
    /* ══════════════════════════════════════════════════════════════
       ★ 말이 안 되는 값을 막는다 ★  (2026-09-21)

       엑셀 열 차례가 어긋난 채 들어온 적이 있다. 수취인이 「2」였고,
       수량이 1914403331002200 이었다. 빠진 칸이 없으니 그대로 통과했다.
       «있으면 된다»가 아니라 «말이 되는가»를 본다.

       조용히 틀린 데이터가 들어가는 것이 안 들어가는 것보다 나쁘다 —
       안 들어가면 사람이 알지만, 틀리게 들어가면 아무도 모른다.
       ══════════════════════════════════════════════════════════════ */
    if (one.qty && parseInt(one.qty, 10) > 9999) {
      빠진것.push("수량이 " + one.qty + " — 열이 밀린 것 같습니다");
    }
    //  수취인이 숫자만이면 이름이 아니다 (수량·단가가 들어온 것)
    if (one.name && /^[0-9]+$/.test(one.name)) {
      빠진것.push("수취인이 「" + one.name + "」 — 이름이 아닙니다");
    }
    //  주소가 너무 짧으면 주소가 아니다. 전화번호가 들어온 일이 있다
    if (one.addr && (one.addr.length < 6 || /^[0-9\-]+$/.test(one.addr))) {
      빠진것.push("주소가 「" + one.addr + "」 — 주소가 아닌 것 같습니다");
    }

    if (빠진것.length) {
      bad.push({ line: i + 1, why: 빠진것.join(" · "), row: one });
      continue;
    }
    clean.push(one);
  }
