...
function _pep_pushCore_(silent) {
  ...
  /*  ★ 2026-09-16: result.detail.pushMissing = _빈칸셈_; 을 지웠다 ★

      > "❌ 대리공급 Push 에러  오류 result is not defined"

      _pep_pushCore_ 에는 result 라는 것이 없다 — 이 함수는 값을 돌려주지
      않는다. 2026-09-15 에 빈칸 점검을 넣으면서 다른 함수의 모양을 그대로
      가져다 붙였다. 그 줄은 try 밖이라 «거기서 그대로 터졌다».

      ★ 그런데 아무도 몰랐다 ★
        같은 날 챗 웹훅이 _secrets.gs 로 옮겨지며 알림이 통째로 죽어 있었다
        (2026-09-16 에 고침). 그래서 이 오류가 9/15부터 하루에 세 번씩
        나고 있었는데 한 번도 눈에 안 띄었다.

      ★ 무엇이 안 돌았나 ★
        전용양식 쓰기와 임시기록은 이 줄 «앞»이라 멀쩡히 나갔다.
        뒤가 통째로 건너뛰어졌다 — 원본 대조 로그, 요약 화면, 그리고
        _sb_syncExclusiveOrders_(전용양식 → DB 동기화).

      _빈칸셈_ 은 아래 요약(빈칸: _빈칸셈_)과 로그가 이미 쓰고 있다.
      이 줄은 있어도 아무 데도 안 닿는 줄이었다.  */
  try {
...
var _PEP_VENDOR_CARRIER_ = {
  ...
  AJ: "한진택배",   // 아주팩 (★ 2026-09-09 로젠 → 한진)
  //   「업체_택배사」탭은 진작 한진(사방넷 004)이었는데 이 폴백만 로젠으로
  //   남아 있었다. 탭을 먼저 읽으니(_pep_loadVendorCarrierTable_) 실제 출고는
  //   한진으로 나가고 있었지만, 탭에서 AJ 줄이 지워지는 날 이 줄이 조용히
  //   로젠으로 되돌린다. 사장님 확인받고 맞췄다.
  AP: "롯데택배",   // 올팩
  YS: "롯데택배",   // 와이에스
  IW: "롯데택배",   // 인터웍스
  JM: "대신택배",   // 제이엠
