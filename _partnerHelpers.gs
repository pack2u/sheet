...
var _PT_ROZEN_FIXED_COL = {
  /* ★ 2026-09-11: 실제 시트를 열어 바로잡았다 ★
       여태 적혀 있던 값(E=주문번호·F=운송장)은 «옛 로젠 양식»이었다.
       지금 탭은 44칸짜리 「주문등록_출력」 양식이라 자리가 통째로 다르다.
       그대로 두면 한 줄도 안 걸리는데 오류는 안 난다 — 0행만 나온다.

       ※ 머리글이 «2행»이다 (1행은 「주문등록_출력(복수건)_출력완료(N)건」).
         이름으로 찾는 것이 먼저고, 이 표는 못 찾았을 때의 바닥이다. */
  name: 6,      // G 수하인
  phone: 9,     // J 수하인 전화
  invoice: 3,   // D 운송장번호
  uid: 18,      // S 주문번호(=사방넷/고유ID)
  item: 14,     // O 물품명
  icode: 34,    // AI 물품코드
  qty: 5,       // F 내품수량
  date: 37,     // AL 파일명 — 20260911.xls 처럼 날짜를 담고 있다
};
