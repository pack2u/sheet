...
var _PT_ROZEN_FIXED_COL = {
  /* ★ 2026-09-15: 시트를 «직접 열어» 다시 바로잡았다 ★
       > "허브에서 송장 수집시 로젠이 수집이 안되지?"  "지금 안되고 있다고."
       > "대리공급 송장은 다 들어왔는데"

       탭이 「집하」 양식으로 바뀌어 있었다. 종전에 적어 둔 44칸
       「주문등록_출력」 자리(D=운송장·S=주문번호)와 통째로 다르다.

       ★ 이 탭에는 머리글이 «없다» ★
         1행부터 바로 자료다. 그래서 이름으로 찾는 길이 아예 막혀 있고,
         D열이 비어 고정열 검증이 헤더 탐지로 넘겼는데 헤더가 없으니
         그것도 실패했다 — 송장 0건. 오류는 한 줄도 안 났다.
         대리공급은 업체 파일에서 걷으므로 멀쩡했고, 자사출고만 사라졌다.
       ...
       ★ 전화는 안 쓴다 ★ P열이 「14***」로 가려져 있어 매칭에 못 쓴다.
         자사출고는 주문번호(J)로 붙으므로 그것으로 충분하다. */
  name: 14,     // O 수하인
  phone: -1,    // P 는 마스킹(14***) — 쓰지 않는다
  invoice: 10,  // K 운송장번호 (451-6945-9705 꼴, 하이픈 포함)
  uid: 9,       // J 주문번호 (사방넷 2162784744 · 전화주문 0915-PH-a1715)
  item: -1,
  icode: -1,
  qty: -1,
  date: 2,      // C 집하일자
...
function _pt_ingestInvoiceSheetTabIntoMap(
  ...
  /*  ★ 머리글이 없는 원천은 «첫 줄»도 자료다 ★  (2026-09-15)
      로젠 「집하」 양식은 1행부터 바로 자료다. 늘 1행을 머리글로 치고
      건너뛰면 그 줄 하나가 조용히 사라진다 — 하필 그게 그날 첫 주문이다.
      머리글로 볼 만한 낱말이 한 줄에도 없고, 0행에 송장이 «있으면»
      그 줄은 자료다. 그때만 0행부터 읽는다. */
  var _startRow_ = 1;
  if (invoiceIdx >= 0 && invData.length > 0) {
    var _r0_ = String((invData[0] || [])[invoiceIdx] || "").replace(/[^0-9]/g, "");
    if (_r0_.length >= 9) {
      _startRow_ = 0;
      _ingestNote_.push("머리글 없음 — 1행부터 자료");
    }
  }
  var invRead = 0;
  for (var i = _startRow_; i < invData.length; i++) {
