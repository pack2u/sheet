...
function partnerCollectOrders(opt_noWriteBack) {
  ...
  /*  ══════════════════════════════════════════════════════════════
      ...
      스마트 수집은 「마지막 수집 이후 «고쳐진» 파일」만 읽는다. 그런데
      기준 시각을 수집이 «끝난» 뒤에 찍고 있었다. 그래서 —

        09:30:00  수집 시작, 아주팩 파일을 읽는다
        09:31:20  업체가 주문 한 줄을 넣는다   ← 파일 수정시각 09:31:20
        09:34:00  수집 끝. 기준 시각 = 09:34:00 으로 저장
        13:00:00  다음 수집 — 아주팩은 09:31:20 < 09:34:00 이라
                  «안 고쳐진 파일»로 보고 통째로 건너뛴다

      그 줄은 영영 안 들어온다. 「한두 건씩」인 까닭이 이것이다 —
      수집이 도는 «그 몇 분» 사이에 들어온 주문만 사라진다.

      ★ 왜 전에는 안 그랬나 ★
        업체가 늘고 파일이 커지면서 수집이 길어졌다. 그 몇 분이 곧 창이다.
        창이 넓어질수록 그 안에 들어오는 주문이 생긴다. 조건이 허술해진 것도,
        6분에 걸린 것도 아니다 — 같은 코드가 느려지면서 드러난 것이다.

      ★ 시작 시각으로 잡으면 ★
        도는 중에 고쳐진 파일은 다음 수집에서 «다시» 읽힌다. 한 번 더 읽는
        것은 손해가 없다 — 이미 들어온 줄은 고유ID로 걸러진다.
      ══════════════════════════════════════════════════════════════ */
  var _수집시작_ = Date.now();
      ...
      for (var ti = 0; ti < collectTabs.length; ti++) {
        ...
        for (var r = 1; r < data.length; r++) {
          ...
          /*  ══════════════════════════════════════════════════════
              ...
              ★ 내용이 같아도 «걷는다» ★  (2026-09-21 · 사장님 지적으로 고침)

                처음에는 「내용까지 같으면 실수로 두 번 넣은 것」이라 보고 뺐다.
                틀렸다 —

                  > "주문한사람이 수량을 잘못 적어서 다시 주문하는 경우가 많거든...
                  >  이런경우 중복으로 걸러내면 문제 발생해"

                적게 적어서 같은 걸 한 번 더 시키면 수취인·전화·품목·수량이
                «다 같다». 내용이 같다는 것은 실수라는 뜻이 아니다.

                ★ 9/18 규칙이 왜 사라지나 ★
                  「고유아이디가 둘이면 빼는 게 맞아」의 까닭은 «송장 매칭이
                  어느 줄인지 못 고른다» 였다. 새 ID 를 주면 그 까닭 자체가
                  없어진다. 남는 위험이 없으므로 버릴 이유도 없다.
                  「못 받는 것보다 두 줄이 낫다」 — 9/18 에 2차 검사에서 이미
                  정하신 그 원칙을 여기에도 그대로 쓴다.

                내용이 같을 때는 «의심»으로 적어 둔다. 진짜 잘못 넣은 것이면
                사람이 허브에서 그 줄을 지우면 된다. 지우는 것은 되지만
                안 들어온 것은 되살릴 수가 없다.

              ★ 허브에 있던 ID 는 손대지 않는다 ★
                업체가 «이미 들어간 주문을 고친» 경우(수량 정정 등)는 줄이
                하나뿐이다. 새 ID 를 주면 한 줄이 허브에 두 번 앉는다.
                파일 «안에서» 겹친 것만 다룬다.
          ...
          if (isDup && _앞줄이쓴ID_) {
            ...
            _같은내용복사_ = (_앞줄키_ === _내용키_);
            _이파일ID_[uid] = _내용키_;
            isDup = false;                     // 이제 새 주문이다
            _새ID발급_ = _같은내용복사_
              ? ('앞줄과 고유ID·내용이 모두 같습니다. 새 ID(' + _옛uid_ + ' → ' + uid +
                 ')를 주고 걷었습니다 — 같은 것을 한 번 더 시킨 것일 수 있고, ' +
                 '잘못 두 번 넣은 것일 수도 있습니다. 진짜 중복이면 허브에서 그 줄을 지우세요')
              : ('앞줄과 고유ID가 겹쳐 새로 발급했습니다 (' + _옛uid_ + ' → ' + uid +
                 ') — 업체가 줄을 복사한 것으로 보입니다');
          }
          ...
          if (_새ID발급_) {
            /*  ★ 걷되 «말한다» ★  둘은 사람이 할 일이 다르다.
                  내용이 다르면 → 복사 버릇일 뿐, 손댈 일 없다 (🆕)
                  내용이 같으면 → 지울지 둘지 사람이 골라야 한다 (⚠)
                한 줄에 한 번만 적는다. 두 곳에 적으면 같은 것을 두 번 보게 된다. */
            if (_같은내용복사_) _의심건수_++; else _새ID건수_++;
            _건너뜀_(_새ID발급_, file.name, r + 1, uid, recipient, code, {
              파일ID: file.id, 탭: tabName, 전화: phoneRaw,
              품목명: itemName, 수량: qtyStr, 짝행: '',
              구분: _같은내용복사_ ? _PO_DUP_IN_ : _PO_DUP_NEWID_,
            });
          }
          if (_의심_) {
            //  ★ 빼지 않는다 ★ 태워 보내고 말만 한다
  ...
  /*  ★ Date.now() 가 아니라 «시작 시각»이다 ★  (2026-09-16 — 위 설명 참고)
      끝난 시각으로 잡으면 수집이 도는 동안 들어온 주문이 다음 수집에서
        ...
        if (_lgTab && _lgTab.getLastRow() > 1) {
          ...
          if (_cUid !== undefined && _cGrp !== undefined) {
            ...
            for (var _sr = 1; _sr < _lgAll.length; _sr++) {
              ...
              /*  ══════════════════════════════════════════════════════
                  ★ 합배송·합포장은 «출력품목명에만» 붙는다 ★  (2026-09-21)

                  > "송장 수집시 적요에 몸통만 뚜껑만 합포장 합배송 내용이
                  >  안딸려오네 이부분도 확인해줘"

                  위 코드는 품목명을 먼저 보고 «비었을 때만» 출력품목명을 본다.
                  그런데 품목명은 거의 늘 차 있어서 출력품목명은 사실상 안 읽힌다.
                  붙는 자리가 갈려 있는 것이 문제다 —
                    ---몸통만 / ---뚜껑만   품목 마스터 이름에 있다 → 품목명
                    ===합배송               세트분리가 «대표 줄»에 붙인다 → 출력품목명
                    ---N개 합포장           세트분리가 붙인다        → 출력품목명
                  그래서 몸통·뚜껑은 들어가고 합배송·합포장만 통째로 빠졌다.
                  2026-09-21 실측(260921-1, 605줄): ===합배송 4줄 · 합포장 25줄이
                  적요에 한 글자도 안 적혔다.

                  ★ 통째로 출력품목명을 쓰지 않는다 ★
                    출력품목명에는 「---개인/스마트스토어」 같은 판매처가 함께 붙고,
                    그건 적요에 들어갈 말이 아니다. 같은 회차에서 출력품목명에만
                    꼬리가 있는 302줄 중 대부분이 그 판매처였다.
                    그래서 «이 둘만» 골라 집는다.
                  ══════════════════════════════════════════════════════ */
              var _tails = [];
              ...
              for (var _ti = 0; _ti < _tails.length; _ti++) {
                var _t2 = _tails[_ti];
                if (!_t2) continue;
                var _sk = _su + "|" + _t2;
