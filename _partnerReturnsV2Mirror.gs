...
function _prv_scheduled_() {
  ...
  /*  ★ 밤마다 «결과»를 본다 ★  (2026-09-16)
      > "다른 주문번호에 송장이 붙어 버리네.. 시스템의 오류가 너무 많아 신뢰가 없네"

      코드마다 그물을 놓는 것은 «아는 구멍»에만 듣는다. 이 점검은 까닭을
      묻지 않고 결과를 본다 — 한 송장이 여러 주문에 붙었는데 합포장이 아니면
      무언가 틀린 것이다. 어느 코드가 그랬든 걸린다.

      여기에 붙이는 까닭: 21:30 은 마감(20:00)이 끝난 뒤이고 대리판매 마감
      (22:00) 앞이다. 그날 것이 다 적힌 뒤에 보는 자리다.
      곁다리라 실패해도 미러는 이미 끝났다. */
  try {
    if (typeof _iod_nightly_ === "function") _iod_nightly_();
  } catch (e4) {
    Logger.log("[송장소유권] 예기치 못한 오류(무시): " + (e4 && e4.message ? e4.message : e4));
  }
}
