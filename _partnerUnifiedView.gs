 ...
 * 모을 일수.
 *
 * ★ 2026-09-02: 14 → 10 으로 줄였다 ★
 *   야간 재생성(22:45)이 5분 예산을 넘겨 기록을 포기하는 일이 생겼다.
 *   9/1 밤이 그랬고, 다음날 CS 대시보드가 하루치를 통째로 잃었다.
 *   시간의 대부분은 일일마감 파일을 여는 데 쓴다 — 일수가 곧 실행시간이다.
 *
 *   실행 창이 좁아서 늘릴 수도 없다. 일일마감(22:00) 뒤, 마감 정리(23:00)가
 *   임시기록을 비우기 전에 끝나야 한다.
 *
 *   ※ 이 값이 CS 주문검색 범위를 그대로 정한다. 10일보다 이전 건은 CS 앱에서
 *     안 나온다. 더 거슬러 봐야 하면 이 값을 올리되, 야간 재생성이 다시
 *     시간초과로 넘어가지 않는지 Chat 알림으로 확인할 것.
 */
var _PUV_DAYS_ = 10;
