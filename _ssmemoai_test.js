...
console.log("\n③ 「안 보냄」을 고르면 안 나간다");
{
  const r = 돌려(안읽히는적요, { 조치: "안 보냄" });
  ok("표식이 붙는다", r.line.적요조치 === "미발송", r.line.적요조치);
  /*  사람이 고르는 말은 「안 보냄」이고, 줄에 붙는 표식은 「미발송」이다.
      바깥 말과 안쪽 표식을 갈라 둔다 — 말은 읽기 쉬워야 하고, 표식은 경로 이름과
      같아야 한다. 위에서 표식이 「미발송」으로 붙는 것을 이미 확인했다. */
  const why = core.ssNonShipReason(
    { 적요조치: r.line.적요조치, 합계: 10000, 품목명: "밀폐용기 500ml", 원본코드: "A100" },
...
console.log("\n④ 「그냥 두기」를 고르면 아무 일도 없다");
{
  const r = 돌려(안읽히는적요, { 조치: "그냥 두기", 주소: "엉뚱한 주소 123" });
...
{
  const r = 돌려(안읽히는적요, { 조치: "주소 바꾸기", 이름: "진지한국밥", 주소: "" });
  ok("주소가 그대로", r.line.주소1 === "경기도 파주시 소라지로 138-53", r.line.주소1);
