function sendSalesToEcount() {
  ...
  var url = 'https://oapi' + zone + '.ecount.com/OAPI/V2/Sale/SaveSaleSlip?SESSION_ID=' + sessionId;
  ...
  for (var key in groups) {
    if (new Date().getTime() - startTime > 290000) { // 4분 50초 경과 시 그룹단위 탈출
      isTimeout = true;
      break; 
    }
...
function createSalesSheet() {
  SpreadsheetApp.getUi().alert('판매전표입력 시트가 없습니다. 시트를 생성하거나 이름을 변경해주세요.');
}
