...
function _bc_mapColumns_(headers) {
  var m = { date: -1, recipient: -1, phone: -1, price: -1, qty: -1, cancel: -1, returnC: -1, itemName: -1, itemCode: -1 };
  for (var i = 0; i < headers.length; i++) {
    var h = String(headers[i] || "").replace(/\s/g, "").toLowerCase();
    if (m.date === -1 && (h.indexOf("주문일자") !== -1 || h === "일자"))
      m.date = i;
    else if (m.recipient === -1 && h.indexOf("수취인") !== -1 && h.indexOf("전화") === -1 && h.indexOf("주소") === -1)
      m.recipient = i;
    else if (m.phone === -1 && (h.indexOf("수취인전화") !== -1 || h.indexOf("전화번호") !== -1))
      m.phone = i;
    else if (m.price === -1 && (h.indexOf("정산금액") !== -1 || h === "단가" || h.indexOf("확정단가") !== -1))
      m.price = i;
    else if (m.qty === -1 && (h === "수량" || h.indexOf("박스수량") !== -1 || h.indexOf("판매수량") !== -1))
      m.qty = i;
    else if (m.cancel === -1 && h === "취소")
      m.cancel = i;
    else if (m.returnC === -1 && h === "반품")
      m.returnC = i;
    else if (m.itemName === -1 && (h === "품목명" || h.indexOf("품목명") !== -1) && h.indexOf("변환") === -1)
      m.itemName = i;
    else if (m.itemCode === -1 && (h.indexOf("이카운트코드") !== -1 || h.indexOf("아카운트코드") !== -1 || h === "품목코드"))
      m.itemCode = i;
  }
  return m;
}
