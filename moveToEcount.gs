function moveToEcount(targetSheetName, injectedSS) {
    ...
    if (invLastRow >= 2) {
      ...
      if (invProdCol !== -1 && invQtyCol !== -1) {
        ...
        for (var i = 0; i < invData.length; i++) {
          var cd = padEcountCode_(invData[i][invProdCol]).toUpperCase();
  ...
  // 매핑에서 요구하는 최대 열 인덱스 찾기 및 자동 열 확장
  var requiredMaxColIdx = 0;
  for (var mIdx = 0; mIdx < explicitMappings.length; mIdx++) {
    if (explicitMappings[mIdx].targetColIdx > requiredMaxColIdx) {
      requiredMaxColIdx = explicitMappings[mIdx].targetColIdx;
    }
  }
  var targetMaxColCount = requiredMaxColIdx + 1;
  var currentSheetColCount = pasteSheet.getLastColumn();
  if (currentSheetColCount < targetMaxColCount) {
    pasteSheet.insertColumnsAfter(currentSheetColCount, targetMaxColCount - currentSheetColCount);
    currentSheetColCount = targetMaxColCount;
  }

  var dataRows = pasteLastRow - dataStartRow + 1;
  var maxPasteCol = currentSheetColCount;
  var fullRange = pasteSheet.getRange(dataStartRow, 1, dataRows, maxPasteCol);
  var fullValues = fullRange.getValues();
  var pasteHeaders = pasteSheet.getRange(headerRow, 1, 1, maxPasteCol).getValues()[0];
  ...
  var skippedCols = {};  // 수식 보호로 인해 동기화에서 스킵된 열 이름 추적
  
  // 🔥 [성능 최적화] O(N^2) 이중 루프를 O(N) 해시맵 탐색으로 압축 (검색 속도 100배 단축)
  var ecDataMap = {};
  for (var i = 0; i < ecData.length; i++) {
    var eCode = padEcountCode_(ecData[i][prodCdCol]).toUpperCase();
  ...
  for (var r = 0; r < dataRows; r++) {
    var code = padEcountCode_(fullValues[r][4]); // E열(인덱스 4) 기준 및 자릿수 패딩 자동 쉴드
    if (!code) continue;
    ...
    var currentEcRow = ecDataMap[codeUpper] !== undefined ? ecDataMap[codeUpper] : -1;
    ...
    for (var d = 0; d < explicitMappings.length; d++) {
       ...
       if (formulaDefense[rule.targetColIdx]) {
           var colLetter = getColLetter_(rule.targetColIdx);
           var hName = String(pasteHeaders[rule.targetColIdx] || "").trim() || colLetter;
           skippedCols[hName] = colLetter;
           continue;
       }
       ...
       if (rule.type === "INV") {
        ...
        // 이카운트 값이 존재하고(undefined가 아님) 현재 엑셀에 적힌 값과 다를 때만 안전 교체
        if (sVal !== undefined && String(tVal) !== String(sVal)) {
  ...
  var skippedColKeys = Object.keys(skippedCols);
  var skippedMsg = "";
  if (skippedColKeys.length > 0) {
     skippedMsg = " (⚠️ 수식보호열 제외: " + skippedColKeys.join(", ") + ")";
  }
  ...
  if (missingCodes.length > 0) {
    ...
    var warnMsg = dbgMsg + " (경고: " + missingCodes.length + "건 이카운트 미조회)" + skippedMsg;
    ...
    var finalMsg = dbgMsg + skippedMsg;
    try { SpreadsheetApp.getActiveSpreadsheet().toast(finalMsg, "성공", 5); } catch(e){}
    pasteSheet.getRange("Z3").setValue(finalMsg).setFontColor(skippedColKeys.length > 0 ? "red" : "black");
  }
...
function moveToEcountSelectedCodes_(codes, targetSheetName, injectedSS) {
  ...
  for (var i = 0; i < (codes || []).length; i++) {
    var raw = padEcountCode_(codes[i]).toUpperCase();
  ...
  if (invSheet && invSheet.getLastRow() >= 3) {
    ...
    if (invProdCol !== -1 && invQtyCol !== -1) {
      ...
      for (var ii = 0; ii < invData.length; ii++) {
        var ic = padEcountCode_(invData[ii][invProdCol]).toUpperCase();
  ...
  for (var r = 0; r < ecData.length; r++) {
    var c = padEcountCode_(ecData[r][prodCdCol]).toUpperCase();
  ...
  var formulaDefense = pasteSheet.getRange(5, 1, 1, maxPasteCol).getFormulas()[0];
  var pasteHeaders = pasteSheet.getRange(4, 1, 1, maxPasteCol).getValues()[0];
  var updated = 0;
  var modifiedCols = {}; // 변경된 열 추적
  var skippedCols = {};  // 수식 보호로 인해 동기화에서 스킵된 열 이름 추적

  for (var pr = 0; pr < dataRows; pr++) {
    var pCode = padEcountCode_(fullValues[pr][4]).toUpperCase();
  ...
  var dbgMsg = "선택 동기화 완료! (" + updated + "건 갱신)";
  var skippedColKeys = Object.keys(skippedCols);
  var skippedMsg = "";
  if (skippedColKeys.length > 0) {
     skippedMsg = " (⚠️ 수식보호열 제외: " + skippedColKeys.join(", ") + ")";
...
function padEcountCode_(code) {
  var s = String(code == null ? "" : code).trim();
  if (!s) return "";
  // 숫자 형식이고 길이가 6자리 미만인 경우 0 패딩 적용
