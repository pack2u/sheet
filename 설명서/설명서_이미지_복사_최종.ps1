# Pack2U 설명서 이미지 복사 스크립트 (최종 수정본)
# 실행 방법: 이 파일을 마우스 오른쪽 버튼으로 클릭 -> [PowerShell에서 실행] 선택

$srcDir = "C:\Users\x299\.gemini\antigravity\brain\6d3331ee-0103-4cd4-bd3b-025618343c6d\.system_generated\click_feedback"
$dstDir = $PSScriptRoot

$mapping = @{
    "01_권한승인_메뉴.png" = "click_feedback_1778685547244.png"
    "02_대리발주_푸시_메뉴.png" = "click_feedback_1778685553860.png"
    "03_뉴파츠_전용양식.png" = "click_feedback_1778685504735.png"
    "04_대리발송_초기화_메뉴.png" = "click_feedback_1778685592319.png"
    "05_발주수집_메뉴.png" = "click_feedback_1778685592319.png"
    "06_협력업체_발주허브.png" = "click_feedback_1778685567100.png"
    "07_송장수집배포_메뉴.png" = "click_feedback_1778685592319.png"
    "08_발주및송장조회_탭.png" = "click_feedback_1778685425823.png"
    "09_월별마감이동_메뉴.png" = "click_feedback_1778685592319.png"
    "10_발주마감_탭.png" = "click_feedback_1778685448690.png"
    "11_취소반품접수_탭.png" = "click_feedback_1778685437631.png"
    "12_취소반품수집_메뉴.png" = "click_feedback_1778685592319.png"
}

Write-Host "--- Pack2U 설명서 이미지 복사 시작 ---" -ForegroundColor Cyan
Write-Host "원본 경로: $srcDir"
Write-Host "대상 경로: $dstDir"
Write-Host ""

if (-not (Test-Path $srcDir)) {
    Write-Host "Error: 원본 경로를 찾을 수 없습니다. 경로를 다시 확인해주세요." -ForegroundColor Red
    Pause
    exit
}

foreach ($dest in $mapping.Keys) {
    $srcFile = Join-Path $srcDir $mapping[$dest]
    $destFile = Join-Path $dstDir $dest
    
    if (Test-Path $srcFile) {
        Copy-Item -Path $srcFile -Destination $destFile -Force
        Write-Host "[성공] $dest 복사 완료" -ForegroundColor Green
    } else {
        Write-Host "[실패] 원본 파일 없음: $($mapping[$dest])" -ForegroundColor Red
    }
}

Write-Host ""
Write-Host "모든 작업이 완료되었습니다! HTML 파일을 새로고침하여 이미지를 확인하세요." -ForegroundColor Cyan
Write-Host "아무 키나 누르면 종료됩니다..."
Pause
