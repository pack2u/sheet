...
function ssMerge(units, cfg) {
  ...
  for (var i = 0; i < units.length; i++) {
    ...
    /*  ★ 샘플끼리는 조건을 안 따진다 ★  (2026-09-21)
        > "여전히 샘플은 10개를 기준으로 합포장을 하는거 같은데?"

        합배송조건 마스터가 샘플을 «크기별로» 열세 가지로 갈라 놓았다 —
        샘플1218 · 샘플1512 · 샘플1914 · 샘플2318 · 평택샘플 · 평택샘플수저 …
        조건이 열쇠에 들어가므로, 크기를 섞어 주문하면 «크기 수만큼» 박스가
        나온다. 9/21 김부남은 샘플 27건이 다섯 박스로 갈렸고, 그래서 한도
        14 는 닿아 보지도 못했다. 사장님이 본 「10개씩 묶인다」가 이것이다.

        샘플은 낱개가 작아 한 박스에 담는 것이 규칙이다(합포장_최대건수_샘플).
        그러니 여기서만 조건을 하나로 본다 — 몇 개까지 담을지는 그 한도가 정한다.
        조건 마스터는 안 건드린다. 거기 크기별 구분은 다른 데서도 쓴다. */
    var 조건칸 = /^\[샘플\]/.test(ssText(u.품목명)) ? '샘플' : u.조건ID;
    var key = u.출고지 + '♦' + ssDeliveryKey(u) + '♦' + 조건칸;
    (groups[key] || (groups[key] = [])).push(u);
  }
