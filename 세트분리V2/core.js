...
function ssNeedsBom_(name, code) {
  ...
  /* ★ 「소분」은 «하나로» 나간다 ★  (2026-09-21)

     > "/소분으로 된것들은 소분이라 하나로 나가는게 맞아"

     소분은 몸통·뚜껑을 미리 맞춰 한 봉지로 담아 둔 것이다. 쪼갤 물건이
     아니므로 BOM 이 없는 것이 «정상»이고, 그걸 두고 「BOM 을 등록하세요」라고
     조를 일이 아니다.

     ★ 규칙이 반쪽만 걸려 있었다 ★
       ssm_load 는 「세트명에 소분이 들어가면 분해하지 않는다」를 이미 쓰는데,
       그 규칙은 BOM «키»를 훑으므로 BOM 이 있는 품목에만 닿는다. BOM 이 없는
       소분 품목은 그 규칙을 만나지도 못한 채 여기서 경고만 맞았다.
       2026-09-21 실측: 소분 품목 2,037개 중 BOM 없는 것 874개,
       그중 268개(판매중 18개)가 회차마다 이 경고를 냈다.

     시끄러운 주의는 안 보게 되고, 안 보는 주의는 없느니만 못하다 — 위 샘플과
     같은 까닭이다. */
  if (n.indexOf('소분') !== -1) return false;
...
function ssMerge(units, cfg) {
  ...
  for (var key in groups) {
    ...
    for (var b = 0; b < boxes.length; b++) {
      ...
      /*  ★ 합포장인가 합배송인가 ★  (2026-09-21)
          > "합포장은 샘플 또는 위생장갑일꺼야.."
          > "대부분 샘플만 합포장이고 소분의 경우는 합배송이 대부분이야"

          합포장 = 낱개를 «한 박스»에 담는다 (샘플·위생장갑처럼 작고 가벼운 것)
          합배송 = 작은 박스 여럿을 «한 송장»으로 보낸다 (소분 등 나머지)

          ★ «모두» 그것일 때만 합포장이다 ★
            하나라도 다른 물건이 끼면 한 박스에 안 들어갈 수 있다.
            잘못 합포장이라 적으면 창고가 한 박스에 몰아 담으려다 막힌다.
            반대로 합배송이라 적어 두면 박스를 따로 싸도 송장은 하나라 손해가 없다.
            헷갈릴 때 덜 위험한 쪽으로 기운다. */
      var 합포장낱말 = ssText(cfg && cfg.합포장_품목낱말) || '위생장갑';
      var 낱말들 = 합포장낱말.split(',');
      var 합포장박스 = true;
      for (var j = 0; j < box.length; j++) {
        box[j].합포장그룹 = boxKey;
        if (box[j].배송비 > maxFee) maxFee = box[j].배송비;
        var _nm3 = ssText(box[j].품목명);
        if (/^\[샘플\]/.test(_nm3)) sample = true;
        var 그것인가 = /^\[샘플\]/.test(_nm3);
        for (var w = 0; !그것인가 && w < 낱말들.length; w++) {
          var _w = 낱말들[w].trim();
          if (_w && _nm3.indexOf(_w) >= 0) 그것인가 = true;
        }
        if (!그것인가) 합포장박스 = false;
      ...
      /* 샘플이 낀 박스는 예전처럼 접어 둔다 (사장님 확인).
         ...
         ★ 여기는 «합배송»이다 — 한때 합포장으로 바꿨다가 되돌렸다 ★  (2026-09-21)
           > "합포장은 샘플같이 낱개 물건을 하나의 박스로 포장을 하는거고..
           >  합배송은 박스가 작으니 2개3개 작은 박스를 하나로 묶어서
           >  한송장으로 보내는것을 말하는거야"

           코드가 대표에 박스수 1 을 적는 것을 보고 「한 박스로 만드니 합포장」
           이라 단정했다가 틀렸다. 장부에 박스를 하나로 적는 것과, 창고에서
           실제로 한 박스에 담느냐는 별개다. 사장님은 이 기능을 줄곧
           «합배송»이라 부르신다 — 소분끼리 묶이는 것이 대부분이고,
           그건 작은 박스 여럿을 한 송장으로 보내는 것이다.

           ★ 합포장은 «다른 자리»에 있다 ★
             ssShippingFee 가 붙이는 「---N개 합포장」이 그것이다 —
             한 줄의 수량이 여럿이라 한 박스에 담는 것.

         ===합배송 은 다른 시스템이 «글자 그대로» 읽는다 —
           _partnerLotteShipCompare.gs  책정배송비 1,900원 판정
