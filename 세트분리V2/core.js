  ...
  for (var i = 0; i < units.length; i++) {
    ...
    if (!면제) {
      ...
      if (예외) {
        ...
        u.보류상세 = 예외.사유 || '';
        var 예업 = 예외.업체코드;
        if (예업 && vendors[예업]) { u.업체코드 = 예업; u.업체명 = vendors[예업]; }
        else if (예업) {
          ssWarn(warnings, '주의', 'PITEM_BAD_VENDOR', u.품목코드 + ' / ' + 예업,
            '「대리발송품목」에 적힌 업체코드가 「대리발송업체」 표에 없습니다. ' +
            '품목 앞 두 글자로 가린 업체(' + (u.업체코드 || '없음') + ')로 둡니다.');
        }
        if (!u.업체코드) {
