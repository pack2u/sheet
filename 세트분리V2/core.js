  ...
  for (var key in groups) {
    ...
    for (var b = 0; b < boxes.length; b++) {
      ...
      /* 샘플이 낀 박스는 예전처럼 접어 둔다 (사장님 확인).
         ...
         ★ 「합배송」이 아니라 「합포장」이다 ★  (2026-09-21)
           > "합포장은 샘플같이 낱개 물건을 하나의 박스로 포장을 하는거고..
           >  합배송은 박스가 작으니 2개3개 작은 박스를 하나로 묶어서
           >  한송장으로 보내는것을 말하는거야"

           여기가 하는 일은 낱개 여럿을 «한 박스»로 만드는 것이다 —
           대표만 박스수 1 을 갖고 나머지는 동봉으로 출력에서 빠진다.
           그러니 합포장이다. 여태 이름표만 「합배송」이라 붙어 있었다.

         ★ 읽는 쪽은 옛말도 받는다 ★
           지난 회차 원장·허브·마감에는 「===합배송」이 그대로 쌓여 있다.
           그걸 고치지는 않는다(사장님: "이미 지난건 굳이 변경할 필요가 없어").
           대신 읽는 곳이 둘 다 받게 해 두었다 —
             _partnerLotteShipCompare.gs  책정배송비 1,900원 판정
             _partnerOrders.gs            허브 적요에 적는 말
           한쪽만 바꾸면 지난 자료가 조용히 안 읽힌다. */
      rep.출력품목명 = (sample ? '[샘플] ' : '') + ssCompressNames(names, !sample) + ' ===합포장' +
