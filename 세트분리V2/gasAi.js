 ...
 * 조치 칸에서 고르는 말.
 *
 * ★ 하는 일을 그대로 적는다 ★  (2026-09-22)
 *   > "말이 어려워 이대로 미발송 아님 이게 도데체 멀 뜻하는지 파악하기가 어렵네"
 *   「이대로 적용」·「아님」은 내가 만든 말이라 무슨 뜻인지 알 길이 없었다.
 *   고르는 사람이 «무슨 일이 일어나는지»를 읽고 알 수 있어야 한다.
 */
var SS_AI_ACTIONS = ['', '주소 바꾸기', '안 보냄', '그냥 두기'];

/*  이미 적어 둔 옛 말도 그대로 받는다 — 바꾸면서 남의 손을 헛되게 하지 않는다. */
var SS_AI_ACTION_ALIAS = {
  '이대로 적용': '주소 바꾸기',
  '미발송': '안 보냄',
  '아님': '그냥 두기'
};
...
function ssm_captureMemoActions() {
  ...
  try {
    ...
    for (var i = 0; i < v.length; i++) {
      ...
      if (!조치) continue;
      if (SS_AI_ACTION_ALIAS[조치]) 조치 = SS_AI_ACTION_ALIAS[조치];   // 옛 말도 받는다
