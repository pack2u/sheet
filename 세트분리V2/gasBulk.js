...
function ssb_collect() {
  ...
  var seenOrd = {};
  /* 원천 표는 여러 날치가 쌓여 있다. 대상일만 남겨 «파일을 작게» 만든다.

     ★ 「다시 올리면 거부된다」가 아니다 ★  (2026-09-14 바로잡음)
       여기 오래 「이미 처리된 건이라 건별 미매칭으로 거부된다」고 적혀 있었다.
       사실이 아니다 —
         > "이미 올라간것은 사방넷에서 알아서 처리해..
         >  쇼핑몰 운송장전송을 하면 미발송에 아무것도 안떠"
       그래서 «여러 날치를 같이 올려도 손해가 없다». 이 자름은 안전장치가
       아니라 크기 조절일 뿐이고, 넓게 잡는 것을 두려워할 이유가 없다.

     ★ 기본값 1 은 주말을 못 넘는다 ★
       달력일로 센다. 월요일에 1 이면 금요일 출고분이 통째로 빠진다.
       금→월을 넘으려면 4 가 필요하다. 상시값은 4 가 맞다. */
    ...
    if (ix['고유ID'] !== undefined && ix['운송장번호'] !== undefined) {
      ...
      for (var g = 0; g < gv.length; g++) {
        ...
        if (uid4 && ix['합포장그룹'] !== undefined) {
          ...
          if (grp4) {
            /*  ══════════════════════════════════════════════════════════
                ★ 합포장그룹만으로 묶으면 «다른 날 주문»이 한 박스가 된다 ★
                2026-09-16

                > "사방넷 송장대량등록 도 오류가 많은듯..
                >  다른 주문번호에 송장이 붙어 버리네.."

                합포장그룹은 ssMerge 가 이렇게 만든다 —
                    출고지 ♦ 배송키(수취인·주소·전화) ♦ 조건ID   (+ 박스번호)
                날짜도 회차도 안 들어간다. 같은 사람이 같은 물건을 다른 날
                시키면 «글자가 똑같은» 그룹이 나온다.

                여기는 대상일 여러 날치를 한꺼번에 읽는다(기본 4일, 주말이면 더).
                ...
                ★ 허브는 이미 회차키를 붙여 묶고 있었다 ★
                  _partnerOrders.gs 의 combinedKeyByUid 는 «회차키 + / + 그룹»이다.
                  같은 일을 하는 두 곳 중 여기만 안 따라왔다.

                회차가 다르면 다른 박스다. 회차키가 없는 옛 줄은 한 칸에 모으되
                그것도 그것끼리만 묶인다.
                ══════════════════════════════════════════════════════════ */
            var 박키4 = (rk || '(회차없음)') + '/' + grp4;
            var 박4 = 박스[박키4] || (박스[박키4] = { rep: '', kids: [], rk: '' });
 ...
 * 도서산간 「조치」 칸에 적어야 하는 말.
 *
 * ★ 보류(미발송) 탭과 «같은 낱말»이다 ★  (2026-09-14)
 *   gasMasters.js ssm_captureManual 이 보류 탭에서 「발송」을 자체 출고로 읽는다.
 *   여기만 아무 글자나 받고 있었다 — 손버릇대로 O 를 적으면 도서산간에서는
 *   나가고 보류 탭에서는 「O 라는 업체로 대리발송」이 됐다.
 *   같은 이름의 칸이 두 가지로 움직이면 사람은 반드시 한쪽을 틀린다.
 */
...
function ssb_islandKeep(vals) {
  ...
  var kept = [head], 나감 = 0, 보류 = 0, 자동 = 0, 확인표 = {}, 딴말 = [];
  for (var r = 1; r < vals.length; r++) {
    ...
    /*  ★ 「발송」 하나로 통일한다 ★  (2026-09-14)
        > "도서산간도 발송으로 통일되게 고쳐줘"
        보류(미발송) 탭의 조치는 「발송」이라고 적어야 자체 출고다. 여기만
        아무 글자나 받고 있어서, 손버릇대로 O 를 적으면 도서산간에서는 나가고
        보류 탭에서는 「O 라는 업체로 대리발송」이 됐다. 같은 칸 이름이
        두 가지로 움직이면 사람은 반드시 한쪽을 틀린다. */
    if (적음 === SSB_ISLAND_OK) { kept.push(row); 나감++; continue; }

    //  한 분에게 한 줄이면 어차피 확인 없이 나간다 — 뭘 적었든 붙들 이유가 없다
    if (혼자) { kept.push(row); 나감++; 자동++; continue; }

    보류++;
    확인표[이름] = (확인표[이름] || 0) + 1;

    /*  ★ 「발송」이 아닌 글자를 적었으면 «그대로 말해 준다» ★
        적어 놓고 안 나가면 사람은 기능이 고장 난 줄 안다. 무엇을 적었는지
        같이 보여 줘야 「아, 발송이라고 써야 하는구나」가 된다. */
    if (적음 && 딴말.length < 5) 딴말.push(이름 + ' 「' + 적음 + '」');
  }
...
function ss_로젠출력탭() {
  ...
  var 도서자동 = 0, 도서확인 = {}, 사람못가림 = false, 도서딴말 = [];
  for (var i = 0; i < names.length; i++) {
    ...
    if (isIsland) {
      ...
      if (!골라.ok) {
        ...
        for (var di = 0; di < (골라.딴말 || []).length; di++) {
          if (도서딴말.length < 5) 도서딴말.push(골라.딴말[di]);
        }
  ...
  if (도서보류) {
    ...
    msg = '⚠ 도서산간 ' + 도서보류 + '건은 안 실었습니다' + NL +
      ...
      '   운임이 두 번 붙습니다. 「조치」 칸에 «발송» 이라고 적고 다시 누르세요.' + NL +
      (확인줄.length ? '   볼 분 : ' + 확인줄.join(' · ') + NL : '') +
      /*  ★ 적어 놓고도 안 나간 줄은 «무엇을 적었는지» 보여 준다 ★
          적었는데 안 나가면 사람은 기능이 고장 난 줄 안다. 적힌 글자를
          같이 보여 줘야 「아, 발송이라고 써야 하는구나」가 된다. */
      (도서딴말.length
        ? '   ⚠ 「발송」이 아닌 글자가 적힌 줄 : ' + 도서딴말.join(' · ') + NL +
          '      보류(미발송) 탭과 같게 «발송» 이라고 적어 주세요.' + NL
        : '') +
      (도서나감 ? '   실은 도서산간: ' + 도서나감 + '건' : '') +
