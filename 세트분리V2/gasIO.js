...
var SSIO_CONFIG_DEFAULTS = [
  ...
  /* ★ 2026-09-09: 기본을 「전체」로 ★
     > "판매현황에 붙여 넣고 세트분리 실행하면 재고등 데이타 불러오고 실행되면 될꺼 같은데"
     예전 기본값 「재고만」은 재고는 따라가지만 **품목·BOM 은 안 읽었다.**
     신상품이 들어온 날 새로고침을 안 누르면 그 주문이 「품목누락」으로
     보류에 빠졌다 — 사람이 기억해야만 맞는 구조였다.
     느려지면 「재고+품목」으로 낮추면 된다. */
  ['실행전_마스터갱신', '전체', '전체 | 재고+품목 | 재고만 | 안함 — 실행할 때 알아서 다시 읽는다. 느리면 재고+품목으로'],
  ...
  /* ★ 2026-09-09: 기본을 「이 시트」로 되돌렸다 ★
     전에는 바깥 시트(판매현황(뉴))에 붙여넣고 그걸 읽어 왔다. 사장님이
     "시트내에서 다 처리되면 좋겠어" 하셔서 비워 둔다 — 비면 이 시트의
     판매현황 탭을 읽는다(ssm_readSales). 바깥 시트를 다시 쓰고 싶으면
     여기에 ID 를 적으면 그때부터 그쪽을 읽는다. */
  ['판매현황_원천시트ID', '', '비우면 이 시트의 판매현황 탭을 쓴다. 바깥 시트에서 가져오려면 그 시트 ID'],
