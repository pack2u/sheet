...
var SSIO_TABS = {
  ...
  입력아이디: '판매현황_고유아이디',
  출력: ['로젠택배', '로젠택배-도서산간', '로젠택배-도서산간(위탁배송)', '대리발송'],
  합배송: '합배송',
  비배송: '비배송',
  사방넷송장: '사방넷송장',
  사방넷등록: '사방넷등록',
  ...
  회차: '회차',
  중복의심: '중복의심',
  /*  규칙이 못 읽은 적요를 AI 가 읽어 «제안»해 두는 자리 (2026-09-22).
      아무것도 저절로 바뀌지 않는다 — 사람이 조치를 골라야 먹는다. */
  적요확인: '적요확인',
  수동조치: '수동조치',
  업체: '대리발송업체',
  설정: '설정',
  ...
  합배송조건: '합배송조건',
  분리예외: '분리예외',
  /* ★ 이 품목은 우리가 안 보낸다 ★  (2026-09-15)
     > "특정상품 이카운트 코드를 넣으면 대리발송으로 빠지게"
     「분리예외」는 세트를 안 쪼개는 표다. 이건 «누가 보내는가»의 표다.
     이름이 닮으면 헷갈리니 하는 일을 그대로 이름에 적는다. */
  대리발송품목: '대리발송품목',
  도서산간시군: '도서산간_시군',
  도서산간우편: '도서산간_우편번호',
  도선료: '도서산간_도선료',
...
var SSIO_CONFIG_DEFAULTS = [
  ...
  ['합포장_최대건수_샘플', '14', '샘플이 낀 박스만 따로. 비우면 위 값과 같게 쓴다 — 샘플은 낱개가 작아 더 담긴다'],
  ['합포장_품목낱말', '위생장갑', '이 낱말이 든 품목«만»으로 묶인 박스는 「합포장」(한 박스)으로 적는다. 샘플은 늘 포함. 나머지는 「합배송」(박스 여럿·송장 하나). 쉼표로 여럿'],
  ['위탁출고지', '대리발송', '재고 부족 시 협력업체로 넘기는 출고지명'],
  ['허용상태', '판매중,임박,특판', '이 상태만 출고. 나머지는 보류로 간다'],
  ['보내는주소', '경기도 평택시 포승읍 성해홍원로 91 팩투유', ''],
  ['대표전화', '031-923-7795', ''],
  ['미발송_적요낱말', '이미출고,출고완료,중복출고,방문수령,방문후수령,직접수령,픽업,주문취소', '적요에 이 낱말이 있으면 미발송으로 뺀다. 쉼표로 여럿. 띄어쓰기는 무시한다. ★「출고」 하나만 넣지 말 것 — 「09/15 출고」 같은 예정 메모까지 빠진다'],
  ...
  ['도선료_통일금액', '5000', '도서산간 도선료를 이 한 값으로 통일한다. 로젠 요율표를 못 받은 동안 표의 롯데 금액(1,000~9,900원)을 쓰느니 한 값이 낫다. 비우거나 0 이면 표를 그대로 쓴다'],
