...
function ss_실행(opts) {
  ...
  try {
    ...
    /*  ★ 전화주문 번호표의 기억 — «회차 확정보다 먼저» ★  (2026-09-22)
        p0922000001 은 그날의 번호표라 «다시 계산»할 수가 없다. 같은 주문에
        아까 준 번호를 그대로 주려면 원장에서 꺼내와야 한다.

        ★ 순서가 중요하다 ★
          바로 아래 회차 지문이 ssNormalize 를 먼저 돌린다. 거기서도 고유ID 를
          매기므로, 기억을 그 «뒤»에 읽으면 지문 계산에서 터진다.
          (지문 자체는 고유ID 를 안 쓰지만, 줄을 만드는 길이 하나뿐이다.)
          지문 계산에서 매긴 번호는 기억에 남아 본 계산에서 그대로 쓰인다 —
          두 번 돌아도 같은 번호다.
    ...
    if (cfg._전화ID기억.왜) {
      /*  못 읽은 채로 돌면 번호를 1 번부터 다시 주게 된다 — 이미 나간 주문과
          같은 번호가 다른 사람에게 간다. 여기서 멈추는 편이 낫다. */
      ssio_alert('전화주문 번호표의 기억(원장)을 못 읽어 멈췄습니다.\n\n' +
        cfg._전화ID기억.왜 + '\n\n그냥 돌리면 번호가 이미 나간 주문과 겹칩니다.');
      return;
    }
...
function ss_송장전파() {
  ...
  //  사람이 출력에서 합친 박스를 주소로 구해 낸 수 / 송장이 여럿이라 못 구한 수
  var 원장주소전파 = 0, 주소전파보류 = 0;
  ...
  if (lg && lg.getLastRow() > 1) {
    ...
    if (li['고유ID'] !== undefined && li['운송장번호'] !== undefined) {
      ...
      /* ── ★ 사람이 출력에서 합친 박스 ★  (2026-09-21) ────────────
         > "이건 실제 사람이 수정해서 만든 합포장 묶음이야"
         > "출력값을 보고 송장입력하는 방식으로.."
         > "세트분리에서는 후보만 만들어 놓는게 어때?"

         세트분리가 낸 합포장그룹은 «후보»다. 실제로 몇 박스로 나갈지는
         사람이 출력 탭에서 정한다 — 다섯 줄을 두 줄로 합쳐 올리는 식이다.
         (합배송조건 마스터가 샘플을 크기별 열세 가지로 갈라 놓아, 한 사람
          샘플 27건이 다섯 박스가 된다. 사람은 그것을 두 박스로 만든다.)

         그러면 사라진 줄의 «대표»는 송장을 못 받고, 그 그룹이 통째로
         미매칭으로 남는다. 위 두 걸음(직접·합포장 전파)으로는 절대 안 채워진다.

         그래서 마지막으로 «같은 회차 · 같은 받는분 · 같은 주소»를 본다.
         거기 온 송장이 하나뿐이면 그 박스로 나간 것이다 — 붙인다.
         여럿이면 어느 박스인지 알 수 없으므로 «안 붙이고» 세어서 알린다.
         잘못 붙이면 고객이 남의 박스를 조회한다. 그럴 바엔 빈칸이 낫다. */
      var 받는칸 = li['거래처명'], 주소칸 = li['주소1'], 회차칸 = li['회차키'];
      if (받는칸 !== undefined && 주소칸 !== undefined && 회차칸 !== undefined) {
        var 자리열쇠 = function (row) {
          return ssText(row[회차칸]) + '\u0001' + ssNorm(ssText(row[받는칸])) +
            '\u0001' + ssNorm(ssText(row[주소칸]));
        };
        var 여럿 = '\u0002여럿';
        var 자리송장 = {}, 자리택배 = {};
        for (var s1 = 0; s1 < lv.length; s1++) {
          ...
          if (자리송장[sk] === undefined) {
            자리송장[sk] = w4;
            자리택배[sk] = li['택배사'] !== undefined ? ssText(lv[s1][li['택배사']]) : '';
          } else if (자리송장[sk] !== w4) {
            자리송장[sk] = 여럿;
          }
        }
        for (var s2 = 0; s2 < lv.length; s2++) {
          if (ssText(lv[s2][li['운송장번호']])) continue;
          ...
          if (!got) continue;
          if (got === 여럿) { 주소전파보류++; continue; }
          lv[s2][li['운송장번호']] = got;
          if (li['택배사'] !== undefined) {
            lv[s2][li['택배사']] = 자리택배[자리열쇠(lv[s2])] || '';
          }
          if (li['송장매칭'] !== undefined) lv[s2][li['송장매칭']] = '주소 전파(사람 합포장)';
          원장주소전파++;
        }
      }
  ...
  var msg = '송장 전파 완료' + NL + NL +
    ...
    ' / 합포장 전파 ' + 전파 + ' / 미매칭 ' + 미매칭 +
    (전화미매칭 ? ' (+전화주문 ' + 전화미매칭 + ')' : '') + NL +
    '  · 원장 기록 · 신규 ' + (원장직접 + 원장전파 + 원장주소전파) +
    ' · 이미 채워짐 ' + 원장기채움 + NL +
    /*  사람이 출력에서 합친 박스를 주소로 구해 낸 것. 0 이면 아예 안 적는다 —
        늘 뜨는 줄은 안 보게 된다. */
    (원장주소전파 || 주소전파보류
      ? '  · 사람 합포장 · 주소로 구함 ' + 원장주소전파 +
        (주소전파보류 ? ' · ★송장이 여럿이라 못 구함 ' + 주소전파보류 : '') + NL
      : '') +
