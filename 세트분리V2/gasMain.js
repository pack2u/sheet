...
function ss_그날판매현황쌓기(runKey) {
  ...
  /*  ══════════════════════════════════════════════════════════════
      ★ 같은 주문이 회차마다 한 벌씩 더 쌓이고 있었다 ★  (2026-09-16)

      > "1차 1,2차 1,2차 1,2,3차 이렇게 쌓이는거 같은데.."

      이 함수는 「판매현황에 올라온 것 = 이번 회차분」이라고 여기고 통째로
      더한다. 그런데 실제 운영은 그렇지 않다 —

      > "판매현황을 전체분을 안넣으면 마지막 차수 판매현황 내용의 송장만
      >  들어오더라구"

      사장님은 회차마다 «그날 전체»를 붙여넣는다. 그래서

        1차 : 1차분           → 탭에 1차분
        2차 : 1차+2차분       → 탭에 1차분(회차1) + 1차·2차분(회차2)
        3차 : 1차+2차+3차분   → 거기에 1·2·3차분(회차3)까지

      1차 주문이 세 벌, 2차 주문이 두 벌이 된다. 그 탭을 그대로 읽는
      일일마감에 같은 주문이 두 번·세 번 들어갔다 — 9/15 마감의 70줄이
      35건 × 2 였던 까닭이 이것이다.
      ...
      ★ 같은 줄이면 «먼저 온 회차»의 것으로 둔다 ★
        위에서 회차키로 줄을 세워 놨다. 그러니 앞에서부터 보며 처음 본 줄만
        남기면, 그 주문은 «실제로 들어온 회차»에 남는다. 뒤 회차에 딸려온
        같은 줄은 버린다.

        열쇠는 맨 뒤 둘(출처·회차키)을 «뺀» 나머지 전부다. 그 둘은 이 탭이
  ...
  ss_옛판매현황탭정리(rk);
  SS_DAILY_복원결과_ = { 줄수: 되살림.length, 회차들: 되살린회차, 겹쳐버림: 겹쳐버림 };
  return add.length;
}
