...
function ssm_captureManual(회차키) {
  ...
  for (var i = 0; i < v.length; i++) {
    ...
    if (적힌원본) {
      ...
      var 원줄 = null, LL = byUid[uid] || [];
      for (var lj = 0; lj < LL.length; lj++) if (LL[lj].원본 === 적힌원본) { 원줄 = LL[lj]; break; }
      if (code && code !== 적힌원본 && (!원줄 || code !== 원줄.코드)) 새코드 = code;
      if (이름 && 원줄 && 이름 !== 원줄.이름) 새이름 = 이름;
      else if (이름 && !원줄) 새이름 = 이름;
