...
console.log("\n[머리글에 택배사 이름이 없는가]");
{
  ...
  /*  ★ 누적 탭의 머리글에서 «칸을 빼지 않는다» ★  (2026-09-16)
      동네배송을 지우며 이 칸도 뺐다가 되돌렸다. ssio_migrateHeader 는
      칸이 «줄면» 탭을 실행이력_구버전_… 으로 밀어내고 빈 탭을 만든다 —
      2026-09-11 에 「오전 1차가 사라져 보인」 그 사고다.
      동네배송은 안 쓰지만 자리는 남긴다. 늘 0 이 들어간다. */
  eq("칸 수 16 — 누적 탭이라 자리를 지킨다", 줄.split(",").length, 16);
  eq("★ 「동네배송」 칸은 «남아 있어야» 한다 (죽은 칸)", 줄.includes("동네배송"), "true");
}
