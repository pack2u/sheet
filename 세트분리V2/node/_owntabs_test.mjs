...
console.log("\n[gasMain] 송장 전파 — 같은 표를 써야 한다");
{
  ...
  const 로젠 = 편찾기(src, "로젠", "uid", "inv");
  const 롯데 = 편찾기(src, "롯데", "uid", "inv");
  /*  2026-09-15: 전파도 「집하」 양식에 맞췄다. 실제 전파 결과가
      「로젠: 머리글을 못 찾았습니다」였고 자사출고가 0건이었다.
      이 시험이 옛 자리(18·3)를 지키고 있어 고치면 깨졌다. */
  eq("로젠 주문번호 = J(9)", 로젠.uid, 9);
  eq("로젠 운송장 = K(10)", 로젠.inv, 10);
  eq("롯데 주문번호", 롯데.uid, 8);
  eq("롯데 운송장", 롯데.inv, 6);
...
/* ═══════════════════════════════════════════════════════════════
   사방넷 주소 — 머리글 이름이 달라도 찾고, 많이 비면 알린다

   > "수집했는데 주소가 80% 이상 안나와"

   출력 탭에서 주소가 «있는» 줄은 전부 0914-PH-…(전화주문)이고, «빈» 줄은
   전부 21626…(사방넷)이었다. 사방넷 건은 주소를 「추가장문형식1」 칸에서
   읽는데, 이카운트 항목 이름이 한 글자만 달라져도 빈 문자열이 온다 —
   오류는 안 나고 주소 없는 송장이 나간다.
   ═══════════════════════════════════════════════════════════════ */
console.log("\n[사방넷 주소] 칸을 못 찾아도 알아채는가");
{
  ...
  const 꺼내기 = new Function(
    "Utilities", "SpreadsheetApp", "Logger", "module",
    core + "\n" + "return { ssPickCol: ssPickCol };",
  );
  const { ssPickCol } = 꺼내기(null, null, { log() {} }, undefined);

  const 후보 = ["추가문자형7", "추가장문형식1", "추가주문자형식1", "추가장문형식", "추가주문형식1", "추가장문1"];
  const 느슨 = /추가.*(장문|형식|문자형)/;

  eq("★ 실제로 쓰는 이름을 먼저 찾는다",
    ssPickCol({ "추가문자형7": 5, "추가장문형식1": 9 }, 후보, 느슨), "추가문자형7");
  eq("★ 「추가문자형7」 을 안다", ssPickCol({ "추가문자형7": 5 }, 후보, 느슨), "추가문자형7");
  eq("★ 바뀐 이름도 찾는다",
    ssPickCol({ "추가주문자형식1": 9 }, 후보, 느슨), "추가주문자형식1");
  eq("★ 후보에 없어도 무늬로 찾는다",
    ssPickCol({ "추가 장문형식 2": 3 }, 후보, 느슨), "추가 장문형식 2");
  eq("★ 아무거나 집지 않는다",
    ssPickCol({ "주소1": 7, "거래처명": 9 }, 후보, 느슨), "");
  eq("느슨한 무늬가 없으면 정확히만",
    ssPickCol({ "추가주문자형식1": 9 }, ["추가장문형식1"], null), "");

  //  ── 배선 ──
  eq("★ 사방넷 주소를 그 칸에서 읽는다",
    core.includes("ssSplit2(g(row, 사방넷주소칸), '/')"), "true");
  eq("★ 못 찾으면 오류로 알린다", core.includes("'SABANG_ADDR_COL'"), "true");
  eq("★ 실제 머리글을 같이 보여 준다", core.includes("판매현황 머리글: ") && core.includes("본이름.join"), "true");
  eq("★ 다른 이름으로 찾았으면 그것도 말한다",
    core.includes("사방넷 주소를 「' + 사방넷주소칸 + '」 칸에서 읽었습니다"), "true");

  //  ── 결과로 잡는 그물 ──
  eq("★ 주소 빈 줄이 많으면 알린다", core.includes("'ADDR_EMPTY'"), "true");
  eq("다섯 줄 넘고 20% 이상일 때만",
    core.includes("주소없음 > 5 && 주소없음 * 5 >= out.length"), "true");
}
