...
function _parseInvoicePairs_(text) {
  ...
  for (var i = 0; i < lines.length; i++) {
    var line = lines[i];
    var m1 = line.match(/^(\d{10,14})\s{1,}(.+)$/);
    if (m1 && m1[2].trim().length >= 1) { pairs.push({ tracking: m1[1], name: m1[2].trim() }); pending = null; continue; }
    var m2 = line.match(/^(.+?)\s{1,}(\d{10,14})$/);
    if (m2 && m2[1].trim().length >= 1 && !/^\d+$/.test(m2[1].trim())) { pairs.push({ tracking: m2[2], name: m2[1].trim() }); pending = null; continue; }
    if (/^\d{10,14}$/.test(line)) { pending = line; continue; }
    if (line.length >= 1 && !/^\d+$/.test(line) && pending) { pairs.push({ tracking: pending, name: line }); pending = null; }
  }
  return pairs;
}
